...
- [ ] AE.7 DNS + Cloudflare zone wiring. *(Operator step — `cloudflared tunnel route dns recon-demo spec.recon-gen.hotchkiss.io` + same for sasquatch hostname. CNAMEs the hostnames to `<tunnel-id>.cfargotunnel.com`. Documented in `deploy/cloudflared/cloudflared.yml.example` header.)*
