...
- [ ] AO.1 (#3, ROOT fix) Store money as integer cents, translate back at the edges (user call 2026-05-21 — the materiality band was rejected as a band-aid; "when real decimals aren't available the best workaround is store in cents and translate back"). **Spike done (2026-05-21):** cents-space drift = 2 genuine vs 116 float-noisy on sasquatch_pr SQLite (≈ what PG's exact NUMERIC shows → cents also restores cross-dialect agreement); blast radius = 5 money columns (`dec202`), ~45 `currency=True` display sites (format layer, unchanged), money read raw at the dataset-projection sites. Proposed design: do money MATH in integer cents (computed_balance SUM, drift — exact, no float) and translate cents→dollars at ONE chokepoint (the `Current*` view / matview projection, `... / 100.0 AS money`), so dashboards / PDF / direct-DB-on-views see dollars unchanged. Footguns: ingest `×100` round-to-int at the `etl_hook`; money literals in SQL/seed flip to cents; divisions (z-score / avg) cast to NUMERIC; one missed `÷100` = silent 100× error → keep translation centralized.
  - [x] AO.1.lock LOCKED: UNIFORM cents (BIGINT money base on ALL dialects; dollars-at-projection). Decided 2026-05-21. Decisive factor = the ETL integrator: an upstream-Oracle → downstream-SQLite pipeline would otherwise need a per-target money representation (cents to SQLite, dollars to Oracle), so the customer feed contract (`<prefix>_transactions.amount_money` / `<prefix>_daily_balances.money`) MUST be dialect-agnostic. Uniform = ONE feed contract (integer cents) regardless of backend; the integrator / `etl_hook` converts dollars→cents (`×100` round-to-int) at the edge. Update `docs/Schema_v6.md` feed-contract to the cents shape.
  - [ ] AO.1.impl Per lock: money cols → BIGINT cents; matview money math in integer cents; `Current*`/matview projection translates to dollars; seed emits cents + `etl_hook` `×100`; money literals → cents; re-lock seeds; SQLite structural drift count 116→2; full suite + 4-way green; commit + patch release.
- [ ] AO.2 (#4) Today's Exceptions magnitude-0 padding (828): split the AO.1 float-noise drift/ledger_drift flowing into the UNION from the known E8 independent-leg residual (read internals); the AO.1 cents fix should clear the float portion. (AO.1's integer-cents math is the systemic fix for every "money-as-REAL" symptom — daily_statement drift KPI, conservation sums, any displayed money math — not just the drift table.)
