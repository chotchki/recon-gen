...
## Phase AE — Mac mini self-hosted demo (Cloudflare-routed, SQLite, daily refresh)

User-asked 2026-05-19. The repo's `recon-gen dashboards` HTTP server already renders the four bundled apps against any L2 instance using a SQLite backend (`X.3` shipped SQLite as a first-class dialect). Pairing that with the Cloudflare-fronted home Mac mini lets the project demonstrate live dashboards at stable hostnames without standing AWS infrastructure — `recon-gen-spec.hotchkiss.io` for the minimal spec_example L2 (the "smallest viable bank" pedagogical fixture, **dashboards-only**) and `recon-gen-sasquatch.hotchkiss.io` for sasquatch_pr (the realistic community-bank flavor, **studio surface with --demo-mode lockdown**). Two SQLite databases, two server processes, two ports, one cloudflared tunnel, both refreshed nightly. Isolated under a dedicated macOS user account, launchd-managed, sandboxed via `sandbox-exec` (the macOS-native chroot analog).

The GitHub self-hosted runner inside the same isolated user closes the deploy loop: when CI publishes a new wheel to PyPI (release.yml), the runner pulls and restages the demo. The runner is not part of the public testing matrix — it's a private deployer.

### Locks ratified 2026-05-19

- **Cloudflare proxy mode**: Cloudflare Tunnel (cloudflared) — no inbound port forward, no public IP exposure, hostnames bind directly to the tunnel.
- **Sandbox shape**: `sandbox-exec` profile per instance — macOS-native chroot analog. Profile allows network bind on the per-instance port, read-only on venv + L2 yaml + cfg.yaml, read-write on the SQLite db + log dir, denies everything else.
- **Refresh cadence + atomic swap**: nightly at 03:00 America/New_York. Build fresh db at `next.sqlite3` (schema apply + data apply + data refresh + audit verify); `mv next.sqlite3 current.sqlite3`; `launchctl kickstart -k` restarts the server (~5s outage). Server keeps last-known-good db if refresh fails. Anchor pinned at 2030-01-01 so `days_ago` window is reproducible.
- **Surface split**: `spec.recon-gen` runs `recon-gen dashboards` (read-only HTTP, no mutation paths). `sasquatch.recon-gen` runs `recon-gen studio --demo-mode` — full studio surface (diagram + L2 view + data-shaping panel + 4 dashboards), but with `--demo-mode` flag disabling all mutation surfaces: Deploy-changes button hidden, L2 yaml save endpoint returns 403, cfg edits blocked, ETL hook trigger disabled. Trainer knobs allowed (`.studio-state.yaml` writes to a per-process tmpdir wiped on restart). Defense in depth: sandbox-exec profile also denies write to L2 yaml + cfg.yaml regardless of UI.
- **Server entry + bind**: both servers bound to `127.0.0.1` only — cloudflared bridges to public; nothing else on the LAN reaches these ports.
- **Update flow**: nightly refresh runs `pip install --upgrade recon-gen` inside the dedicated user's venv before rebuilding. CI doesn't push; runner pulls. Optional pin via `RECON_GEN_PIN_VERSION` env var.
- **GitHub runner role**: registered with label `mac-mini-demo`; queued only for `.github/workflows/demo-publish.yml` (new). Not a generic build runner.
...
- [ ] AE.1 macOS user + filesystem layout. Create `recon-demo` user (no admin, no GUI login). `~recon-demo/` layout: `venv/` (per-user uv venv), `spec_example/{config.yaml, l2.yaml, current.sqlite3, next.sqlite3, logs/}`, `sasquatch_pr/{...}`, `runner/` (GitHub runner install), `tunnel/{cloudflared.yml, credentials.json}`. Install path includes `~recon-demo/venv/bin` only. Owns nothing outside `~recon-demo/`. Document via `docs/operations/mac-mini-host-setup.md`. *(Operator step — runs on the Mac mini, not in this repo.)*
...
- [ ] AE.2.b.chrome (deferred from AE.2.b first cut) — UI cosmetics for demo-mode. (1) `_render_home_page` + `_render_data_page` + `_render_diagram_page` accept `demo_mode: bool` and emit a top-of-page banner: "Read-only demo — mutations disabled" with a link to the operator's docs explaining the lockdown. (2) Deploy-changes button is conditionally rendered (hidden when demo_mode). (3) L2 editor save / create / delete buttons in the editor card chrome are conditionally rendered (hidden when demo_mode — the route is gone anyway, but the button shouldn't appear). Browser smoke: assert banner present, Deploy button absent in DOM. Defer-rationale: the route-level lockdown (AE.2.b first cut) is the load-bearing safety; chrome cosmetics are friendlier UX but not a correctness gate.
...
- [ ] AE.7 DNS + Cloudflare zone wiring. *(Operator step — `cloudflared tunnel route dns recon-demo recon-gen-spec.hotchkiss.io` + same for sasquatch hostname. CNAMEs the hostnames to `<tunnel-id>.cfargotunnel.com`. Documented in `deploy/cloudflared/cloudflared.yml.example` header.)*
...
- [ ] Inbox.0 Inbox
  - [ ] Inbox.1 AB.7.1a follow-on: clean up docs build chain — persona leaks + dead anchors
