...
- [ ] **Y.7-followup — Oracle browser layer triage (NEW 2026-05-11; root cause identified).** `sp_or_aw` browser layer fails (6 fail + 6 err across two runs). **Root cause from the run-`20260511T0550…`/`sp_or_aw` artifacts: `RuntimeError: Oracle stmt #33 failed (ORA-12006: materialized view "ADMIN"."SP_OR_AW_CURRENT_TRANSACTIONS" already exists)`** — a schema *re-apply* failure on Aurora Oracle. The schema DROP list DOES include `drop_matview_if_exists(f"{p}_current_transactions", dialect)` (`common/l2/schema.py:160`), so the idempotent DROP *should* clear it before the CREATE. The most likely trigger: **the FIRST Y.7 chain run (`up_to=browser --scenarios=sp --targets=lo,aw`) got SIGTERM-killed mid-flight** (the `nohup &` didn't survive the Bash-tool shell exit) — that interrupted `sp_or_aw`'s schema apply partway, leaving Aurora Oracle in a half-applied state the next run's idempotent DROP can't cleanly recover (a dropped-but-not-fully-dropped matview, a stuck refresh, or a dependent-index lock). Subsequent failures cascade: schema apply errors → matviews stale/missing → browser tests can't render → TreeValidator mismatches + selector timeouts; the 6 `test_invariant_three_way_agreement[oracle-*]` ERRORS are the `seeded_audit` fixture failing on the same broken schema. **Fix steps:** (1) manually drop ALL `sp_or_aw_*` objects on Aurora Oracle (tables + matviews + views + indexes) so the next `schema apply` starts clean — `BEGIN FOR r IN (SELECT object_name, object_type FROM user_objects WHERE object_name LIKE 'SP_OR_AW_%') LOOP EXECUTE IMMEDIATE 'DROP '||r.object_type||' '||r.object_name||CASE WHEN r.object_type='TABLE' THEN ' CASCADE CONSTRAINTS' ELSE '' END; END LOOP; END;` (or `DROP MATERIALIZED VIEW` for matviews). (2) Re-run `./run_tests.sh up_to=browser --variants=sp_or_aw` — if it goes green, the root cause WAS the killed-chain residue and there's no code bug; just harden `drop_matview_if_exists` on Oracle to also handle the half-dropped state (e.g. `DROP MATERIALIZED VIEW` then fall back to `DROP TABLE` of the same name). (3) If it still fails after a clean DB, it's a genuine Oracle deploy/render regression — pull the deployed-dashboard definition + screenshots and dig. Blocks the v9.0.0 "Phase Y done" cut; does NOT block the v8.8.0aN alpha (release pipeline is PG-only — v8.8.0a27 shipped clean).
