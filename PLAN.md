...
- [ ] **Y.2.gate.b — Design the runner.** Spike (b.0) returned. Substrate locked: pytest-as-orchestrator + ~400 LOC Python wrapper at `quicksight_gen/_dev/runner.py`, surfaced via `./run_tests.sh` bash shim. Each design item below tracks against that substrate.
    ...
    - [x] **Y.2.gate.b.14 — Bash entry-point shim minimum viable shape (LOCKED 2026-05-07).** ~12 lines of bash following `run_e2e.sh` style: shebang, `set -euo pipefail`, `cd` to script dir, venv-exists check, `exec .venv/bin/python -m quicksight_gen._dev.runner "$@"`. SIGINT propagates through `exec`; no trap needed. No arg parsing in bash — orchestrator owns it. `.venv/bin/python` direct (per memory) over `uv run` to skip the ~200ms startup overhead. Full source captured in spike doc §6.
