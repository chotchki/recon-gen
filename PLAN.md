...
- [ ] BC.11 - **CI fix: integration-pg sq_pg_lo `StringDataRightTruncation: value too long for type character varying(100)`.** Failing on every main-branch CI run post-v11.18.0 (run 26373578977 is the latest). The offending INSERT: `tx-failed-cust-0001-snb` row, prefix `qsgen_sq_pg_lo`, `metadata = '{"generator":"FailedTransactionGenerator","scenario_id":"build-full-seed-qsgen_sq_pg_lo"}'` (89 chars — fits varchar(100)), but SOME column in `<prefix>_transactions` is `varchar(100)` and one of the row's values overflows. spec_example variant passes (the prefix is shorter); sasquatch fails. Diagnostic-first:
  - [ ] BC.11.1 - Identify the column. Grep `varchar(100)` in `common/l2/schema.py` + base-table DDL emitters; cross-ref against the 17 columns in the failing INSERT's column list to find the overflow. Likely `id` (`tx-failed-cust-0001-snb` is 22 chars + prefix-injected fields in sasquatch are longer) or `metadata` (sasquatch personas add metadata).
  - [ ] BC.11.2 - Fix. Either widen the column DDL (cheap; varchar(255) common floor) OR shorten the upstream value (preferred if the value is generator-shaped). Document the choice — production schemas exist with the v1 width; widening may need a migration story.
  - [ ] BC.11.3 - Add a CI regression test that exercises the sasquatch prefix path explicitly.
