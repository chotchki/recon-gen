...
- [ ] AE.2.b Studio `--demo-mode` flag (the lockdown gate). New CLI flag on `recon-gen studio` that, when set: (1) hides the Deploy-changes button from the rendered chrome; (2) returns 403 on POST to L2 yaml save / cfg edit / ETL hook trigger endpoints; (3) writes `.studio-state.yaml` to a per-process tmpdir (passed via env var, wiped on restart) instead of `<cfg.parent>/.studio-state.yaml`; (4) banner across the top of every page: "Read-only demo — mutations disabled". Unit tests: every mutation route returns 403 in demo-mode; every read route returns 200. Browser smoke: deploy button absent from DOM. Both sandbox profiles assume demo-mode is on but don't depend on it — sandbox-exec deny-write is the load-bearing safety; --demo-mode is the friendlier UX cut.
