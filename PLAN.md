...
- [ ] AM.2 - Convert diagram chrome + studio-header + data-knob panel to raw Tailwind utilities per L2. Drop `diagram.css` (chrome rules only — SVG `.topology-svg` / `.node` / `.edge` stay per L4) + `data.css` (per L3). Update `@source` to include `_studio_routes.py`.
- [ ] AM.3 - Verify: re-screenshot create / edit / diagram / dashboards before↔after for visual parity; AI browser tests (account / rail multi-select / BB.2 inline JS) stay green; HTTP dogfood (14 variants) stays green; output.css regenerates via `scripts/build_app2_css.py` with the expanded `@source` set.
- [ ] AM.4 - Drop `editor.css` entirely (no irreducible remainder — pure chrome). Drop `data.css` entirely. Reduce `diagram.css` to SVG-only rules (irreducible — semantic per L4); rename to make scope obvious (e.g. `diagram-svg.css`). Update asset links in `_studio_assets/__init__.py`-style references. Commit + Phase AM history one-liner.
