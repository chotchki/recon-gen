...
- [ ] AI.9 - **Dataclass-field type smell surfaced by AI.2.d.2 (2026-05-25)**: `RoleExpression = Identifier | tuple[Identifier, ...]` on `SingleLegRail.leg_role` / `TwoLegRail.source_role` / etc. is a union, but the L2 loader always normalizes bare strings to single-element tuples — so the bare-Identifier branch is dead in practice. The HTTP driver's `create_form_data` encoder iterates `value` as a tuple under `multi_select` kind; passing a bare Identifier explodes character-by-character (Identifier is a `str` subclass). Either (a) drop the bare-Identifier branch from the type alias (make wrong unrepresentable — loader's normalization becomes the type's invariant), OR (b) coerce bare-Identifier → tuple in the encoder. (a) is the Rust-shaped fix; AI.2.d.2 currently works around by constructing tuples explicitly in the test. Same pattern as AI.8: AI.2.d.2 found the smell that the HTTP TestClient couldn't.
