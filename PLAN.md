...
- [ ] **Y.2.gate.b — Design the runner.** Composed of a leading spike (b.0) plus per-decision sub-tasks. LOCKED items have been decided in audit §7.x and just need to flow into the design doc.
    - [ ] **Y.2.gate.b.0 — SPIKE: runner-implementation language evaluation** (audit §7.3 UNLOCKED 2026-05-07). The earlier "shell script" lock was vibes-not-spike. Constraint set: process management, cross-variant + within-layer parallelism, JSON capture/diff loop, dependency probe (no state file), small-codebase. Candidates: pure bash + jq, pure Python, `just` (Rust task runner), pytest-as-orchestrator + thin Python wrapper, `nox`/`tox`/`mise`, Click subcommand. Acceptance: write `docs/audits/y_2_gate_b_0_runner_lang_spike.md` comparing options on the constraint set + a recommendation. Recommendation locks §7.3; b.1+ design work picks up under the chosen substrate. **Anti-goal: avoid the "huge second code base" trap** — if no candidate clearly fits, the spike's job is to flag that and propose a hybrid (e.g., shell + Python helpers) before locking.
    - [x] **Y.2.gate.b.1 — Variant axis catalog (LOCKED 2026-05-07).** Each layer can be parameterized — same layer × N variants; runner multiplexes and rolls per-(layer, variant) status. Axes: **Dialect** (PG / Oracle / SQLite) — layer 3a all three, layer 3b PG + Oracle, layers 4-6 PG-AWS + Oracle-AWS, layers 1-2 none. **L2 instance** — `spec_example` (default) / `sasquatch_pr` (CI) / future. **Fuzz seed** — first-class property-testing axis, default = 1 random seed/run, operator opts into N via `--fuzz-seeds=N` (audit §7.11). **Browser e2e target** — `qs | app2` at the merged layer 6/7, parallel with fast-fail (audit §7.10/§7.12). **Future**: per-app subset, per-region.
    - [x] **Y.2.gate.b.2 — Container-backed isolation for local-DB variants (LOCKED 2026-05-07).** Layer 3a/3b dialect variants run in per-variant Docker containers (PG, Oracle), not against live Aurora. `testcontainers-python` is the leading candidate (typed, cleanup on exit). Aurora reserved for layer 4+ (deploy / e2e). Audit §7.5.
    - [x] **Y.2.gate.b.3 — App2-as-early-gate chain placement (LOCKED 2026-05-07).** App2 slots in as layer 3.7 — runs after DB smoke, before QS deploy; same dataset SQL against local Docker; catches ~80% of bug classes. At the merged 6/7 layer, App2 + QS run in parallel with fast-fail. Audit §7.10. Strategic ownership lives in X.2.j.
    - [x] **Y.2.gate.b.4 — Per-run output isolation shape (LOCKED 2026-05-07).** Every artifact under `runs/<run-id>/...`; run-id = `<utc-ts>-<short-sha>[-dirty]`. Captures: generated JSON, screenshots, failure manifests, coverage data, top-queries dumps, container logs, browser JS console + Playwright traces/HAR/video (failure-only default; `--trace-all` for verbose). **Every L2 YAML always copied — fuzzed + static** so a static-fixture edit between runs is visible same way fuzz seeds reproduce. Last N=20 runs kept; auto-prune. `runs/` gitignored. "Disk is cheap compared to time."
    - [x] **Y.2.gate.b.5 — Timing + hash capture/diff design (LOCKED 2026-05-07).** Runner records per-(layer, variant, test) timings to `runs/<run-id>/timings.json` + sibling `hashes.json`. On next invocation, reads most-recent matching prior run; reports `step took X (was Y, ±Z%)`. ⚠ threshold ±50% (configurable). **Single-source-of-drift principle** — any stable artifact (timing, seed hash, output bytes, row counts) lives in the runs dir and gets diffed; the global SHA256 lock collapses into this loop (lands with `Y.2.gate.c.13`).
    - [x] **Y.2.gate.b.6 — CI artifact upload (LOCKED 2026-05-07).** CI uploads `timings.json` per-job so cross-CI-run drift is visible. Same shape as existing `coverage-data-py3.13` / `e2e-pg-api-top-queries-${run_id}` artifacts.
    - [x] **Y.2.gate.b.7 — Stop-on-first-failure semantics (LOCKED).** Yes — no point running layer N+1 if layer N failed. Within a layer, failures across variants surface as a per-cell roll-up; one variant failing doesn't kill the others in the same layer.
    - [x] **Y.2.gate.b.8 — Skip-if-already-green policy (LOCKED).** Default = always re-run cheap layers; offer `--skip-cheap` flag for tight iteration loops. Cache lives in `.run_tests_cache/<sha>.json` so a `git commit` invalidates.
    - [x] **Y.2.gate.b.9 — Cross-layer parallelism rules (LOCKED).** Cross-layer = sequential (chain semantics require it; pipelining complicates failure attribution). Within-layer-within-variant = xdist. Cross-variant within a layer = parallel (per-variant Docker / per-`L2Instance` AWS resource set). App2 + QS at layer 6 = parallel with fast-fail.
    - [ ] **Y.2.gate.b.10 — Dirty-state detection on deploy.** Decide: should `up_to=deploy` refuse to fire if there are uncommitted changes? Lean yes (the `[-dirty]` run-id suffix already signals it; refusing the deploy step is the next escalation), but the spike (Y.2.gate.b.0) may shape the answer. Locks under b.0 or in a separate sub-task.
    - [ ] **Y.2.gate.b.11 — Variant-to-layer enumeration (audit dependency).** Y.2.gate.a's audit doc enumerates which variants apply to which layers in the canonical layer table. Confirm complete + flowing into the runner config; gaps go back into the audit.
- [ ] **Y.2.gate.c — Implement the runner.** Implementation language locked by `Y.2.gate.b.0` spike (until then, treat any "shell script" / "Python module" wording elsewhere as preliminary). Per-deliverable sub-tasks:
    - [ ] **Y.2.gate.c.1 — Per-run directory creation + run-id format.** `runs/<run-id>/` created at session start; run-id = `<utc-ts>-<short-sha>[-dirty]`. All subsequent artifacts route under it.
    - [ ] **Y.2.gate.c.2 — Timings + hashes capture.** `runs/<run-id>/timings.json` + `hashes.json` (or merged) recorded per (layer, variant, test).
    - [ ] **Y.2.gate.c.3 — Drift-diff report.** On invocation, scan prior `runs/*/timings.json` for matching SHA (else most-recent); report `step took X (was Y, ±Z%)` with ⚠ at ±50%.
    - [ ] **Y.2.gate.c.4 — `runs/` gitignored + auto-prune past N=20.** Last N=20 runs retained.
    - [ ] **Y.2.gate.c.5 — Stop-on-first-failure across layers.** Chain semantics enforced.
    - [ ] **Y.2.gate.c.6 — Within-layer parallelism.** xdist within-variant; cross-variant fan-out at layer 3 (per-variant Docker); App2 + QS targets run in parallel at layer 6 with fast-fail.
    - [ ] **Y.2.gate.c.7 — Flag plumbing.** `--up-to=<layer>`, `--variants=<set>`, `--only=<test-id>`, `--skip-cheap`, `--fuzz-seeds=N`, `--keep-on-failure` (per `Y.2.gate.f.5`), `--trace-all` (per `Y.2.gate.b.4`).
    - [ ] **Y.2.gate.c.8 — Dependency probe (no state file, LOCKED §7.12).** Layer dispatch probes the actual dependency (`aws sts get-caller-identity`, `psycopg.connect`, `docker ps`); stale state files were explicitly rejected.
    - [ ] **Y.2.gate.c.9 — General-sweep cleanup subcommand.** `./run_tests.sh sweep` (or equivalent) cleans orphan resources tagged `ManagedBy:quicksight-gen`. Powers the `Y.2.gate.f.8` (replaces `scripts/sweep_harness_orphans.py`) + cleans up after `--keep-on-failure` sessions.
    - [ ] **Y.2.gate.c.10 — Top-queries auto-capture (per `Y.2.gate.f.4`).** Runner dumps top queries per touched DB (local + remote) into `runs/<run-id>/db/<dialect>/top-queries.md` automatically every run; replaces `scripts/dump_top_queries.py`.
    - [ ] **Y.2.gate.c.11 — Browser trace capture.** Playwright JS console + network traces / HAR / video / screenshots land under `runs/<run-id>/browser/` (failure-only default; `--trace-all` opt-in).
    - [ ] **Y.2.gate.c.12 — L2 YAML always copied.** Every L2 instance the run touches lands at `runs/<run-id>/l2/<instance-or-seed>.yaml` — fuzzed AND static.
    - [ ] **Y.2.gate.c.13 — `--seed-density=N` tunable generator parameter** (audit §7.13 LOCKED 2026-05-07; lands with the hash-lock move). Once the hash lock lives in `runs/<run-id>/hashes.json` (instead of a frozen-forever value pinned by `tests/data/test_l2_baseline_seed.py`), drift detection happens at the run-pair level — operators can tune the generator (e.g. heavier seeds for nightly, lighter for fast iteration) without re-locking a global hash. Sub-tasks:
        - [ ] **c.13.1 — Plumb `--seed-density=N` (or equivalent param) through `emit_full_seed`.**
        - [ ] **c.13.2 — Drop the global SHA256 lock in favor of the per-run hash recorded in `runs/<run-id>/hashes.json`.**
        - [ ] **c.13.3 — Regression guard test:** previous default density produces same byte-output as before the change.
...
- [ ] **Y.2.gate.i — AWS auth refresh.** Constant `aws sso login` friction is automated away. Claude should never have to ask the user to "re-login" mid-flow — the runner handles it.
    - [ ] **Y.2.gate.i.1 — Design.** Decide pluggable-login shape so operators on `aws-vault` / role-assumption / static keys can wire their flow. Config lives in `run/auth.yaml` (or similar); default = `aws sso login` with auto-detected profile. Specify retry policy (single retry after refresh; second failure surfaces real error). Pre-flight check: `aws sts get-caller-identity` — refresh on `ExpiredTokenException` / `UnrecognizedClientException`.
