...
- [ ] AI.14 - **BB.2 form completeness + structured pickers** (consolidates AI.14 + AI.15 per user 2026-05-25; defer until post-AM Tailwind sweep so visual chrome is unified first). One scope with three sub-pieces — they all touch the same set of forms and share the per-rail-checkbox-picker design. **AM.0 callback (2026-05-25)**: when the textarea-elimination work fires, the new structured pickers should use the same theme-driven surface vocabulary AM landed — `bg-surface-alt` for "subtle disabled-state" (e.g. empty picker placeholder, "no rails to pick yet" hint). Don't reach for `bg-slate-50` or any palette-bypassing color; the `--color-surface-alt` token IS the theme-driven escape hatch.
  - **(a) BB.2 sub-form completeness** — currently the create-new sub-form for aggregator-rail / TT reconcilers only renders validator-required minimums (subtype + cadence + role + direction + expected_net + origin for aggregator; name + expected_net + transfer_key + completion for TT). Optional fields (`metadata_keys`, `amount_typical_range`, `firings_typical_per_period`, `description`, `posted_requirements`, `max_pending_age` / `max_unbundled_age`, `bundles_activity`) don't render → an operator's create-new'd reconciler can never match a reference that sets those, and `tests/e2e/test_studio_dogfood_browser.py::test_browser_full_create_l2_structural_equality` (piece 3) stays skipped. Widen the sub-form to expose every rail / TT field. Busy sub-form but completeness wins for the dogfood proof + the operator's real-world need.
  - **(b) Replace free-form textareas with structured pickers** — AI.10's MVP `leg_rail_xor_groups_text` (one group per line, comma-separated rail names) is typo-prone + has no autocomplete / validation feedback / visual hint at which rails are eligible. Replace with a per-rail-checkbox-group picker (mirrors the existing `multi_select_groups` field-kind in `_FIELD_SPECS_BY_KIND`'s TT.leg_rail_xor_groups but as a rail-create-form affordance). Same pattern applies to other textareas: `metadata_keys`, `posted_requirements`, `metadata_value_examples` — audit + structurize each.
  - **(c) Driver alignment** — once BB.2 exposes every field, `StudioBrowserEditorDriver` doesn't need the post-create backfill workaround I prototyped (the `create_form_data` payload reaches the form natively). Drop the driver-side complexity; piece 3 unblocks naturally.
  Driver-side interim workaround (option (b) of the original AI.15 framing — post-create wave that EDITs each create-new'd reconciler with full field data) is fully implementable today and unblocks piece 3 immediately, but masks the operator-facing gap. Per user 2026-05-25: skip the workaround, plan + ship the real fix post-AM.
...
- [ ] AM.1 - Convert editor screens (create / edit / list / singleton / read-card) to raw Tailwind utilities per L2. Drop the `editor.css` rules. Update `@source` to include `_studio_editor_routes.py`. **Fold in the AI.2.e part-2 stretch** — richer subtype-aware per-field requirement hints beyond the existing `*` markers + entity intro (e.g. a subtype requirements banner on the rail form). *Status 2026-05-25: chrome / form-field migration steps 1-7 landed (helpers + create-page + edit-page + list-page + singleton-page + rail-subtype-picker + `_render_field` + `_render_multi_select_groups_field` + `_render_xor_group_row` + `_render_chain_children_field` + `_render_reconciler_section` + `_render_read_card` + `_render_read_value` bullets). `editor.css` link dropped from every editor page-shell (still loaded by `_studio_routes.py` chrome — drops in AM.2). 22 helper-pin tests + full unit suite (2793) + 3/3 studio browser dogfood tests green. Drivers (`tests/e2e/_drivers/studio_browser_editor.py`, `tests/e2e/_drivers/studio_editor.py`) switched to user-facing locators (`get_by_role("alert")`, `article:has(h3:has-text(...))`) — never CSS classes — after user feedback 2026-05-25; saved as `feedback_browser_drivers_user_facing_locators`. Error blocks now carry `role="alert"` (ARIA semantic; what assistive tech reads). `entity-card` + `entity-card-title` retained as marker classes for the home-page JS until AM.2 migrates it.* **Pre-tick decision wanted:** the AI.2.e part-2 subtype-requirements-banner stretch wasn't shipped (the chrome work was always the load-bearing part; banner prose is second-order). Either accept the box-tick as "Tailwind chrome migration complete, banner prose remains a separate ticket" OR fold the banner in before ticking. Per no-silent-defer: flagging for sign-off rather than burying.
