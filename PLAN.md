...
- [ ] AE.2 Sandbox profiles. New `deploy/sandbox/recon-demo-spec.sb` (dashboards surface) and `deploy/sandbox/recon-demo-sasquatch.sb` (studio --demo-mode surface, locked down further). Both: allow `network-bind` on the per-instance loopback port; allow file-read on `~recon-demo/<instance>/{config.yaml,l2.yaml}` + venv + system Python stdlib; allow file-write on `~recon-demo/<instance>/{current.sqlite3,logs/}`; **deny file-write everywhere else** (especially L2 yaml + cfg.yaml — defense in depth against a studio bug that bypasses --demo-mode). Sasquatch profile additionally allows file-write on a per-process tmpdir for `.studio-state.yaml`. Test the profile by running `sandbox-exec -f recon-demo-X.sb /bin/sh` and trying disallowed ops (should fail). Cross-reference: `man sandbox-exec`, the Chromium sandbox profiles as known-working examples.
...
- [ ] AE.3 SQLite per-instance initial provisioning. `scripts/provision_demo_instance.sh <instance_name> <l2_yaml> <port>`: writes `config.yaml` (dialect=sqlite, deployment_name=demo-<instance>, db file path), copies `<l2_yaml>` to `l2.yaml`, runs `recon-gen schema apply --execute` + `data apply --execute` + `data refresh --execute` against the new SQLite db. Validates via `recon-gen audit verify` (sanity probe — L1 invariants compute cleanly). Two invocations: `spec_example` from `tests/l2/spec_example.yaml` on port 8401; `sasquatch_pr` from `tests/l2/sasquatch_pr.yaml` on port 8402.
- [ ] AE.4 launchd: server jobs (split surface). Two plist files in `~recon-demo/Library/LaunchAgents/`. `io.hotchkiss.recon-demo.spec.plist` runs `sandbox-exec -f recon-demo-spec.sb -- venv/bin/recon-gen dashboards -c spec_example/config.yaml --l2 spec_example/l2.yaml --port 8401 --bind 127.0.0.1`. `io.hotchkiss.recon-demo.sasquatch.plist` runs `sandbox-exec -f recon-demo-sasquatch.sb -- venv/bin/recon-gen studio --demo-mode -c sasquatch_pr/config.yaml --l2 sasquatch_pr/l2.yaml --port 8402 --bind 127.0.0.1`. Both `KeepAlive=true`, `RunAtLoad=true`, log stdout/stderr to `<instance>/logs/server.{out,err}.log` (rotated weekly via launchd's built-in log facilities).
- [ ] AE.5 launchd: nightly refresh job. `~recon-demo/Library/LaunchAgents/io.hotchkiss.recon-demo.refresh.plist`: fires daily at 03:00 ET. Script: per instance — (1) `pip install --upgrade recon-gen` inside venv (or pin via `RECON_GEN_PIN_VERSION`); (2) build fresh db at `next.sqlite3` (schema apply + data apply + data refresh + audit verify); (3) atomic `mv next.sqlite3 current.sqlite3`; (4) signal the per-instance server process via `launchctl kickstart -k` (kill + restart, ~5s outage). Logs to `<instance>/logs/refresh.{out,err}.log`. Exit code propagates to launchd; failures trigger no-op (server keeps last-known-good db).
- [ ] AE.6 Cloudflare Tunnel install + ingress config. `brew install cloudflared` under the `recon-demo` user. `cloudflared tunnel create recon-demo`. `tunnel/cloudflared.yml`: `ingress: [{hostname: spec.recon-gen.hotchkiss.io, service: http://127.0.0.1:8401}, {hostname: sasquatch.recon-gen.hotchkiss.io, service: http://127.0.0.1:8402}, {service: http_status:404}]`. Run as launchd service `io.hotchkiss.recon-demo.tunnel.plist` (KeepAlive=true). Store credentials.json in `~recon-demo/tunnel/` (mode 0600).
- [ ] AE.7 DNS + Cloudflare zone wiring. In the `hotchkiss.io` zone, add CNAME records: `spec.recon-gen` → `<tunnel-id>.cfargotunnel.com`, `sasquatch.recon-gen` → same. Proxied (orange-cloud) ON. Tunnel handles the TLS termination. Document in `docs/operations/cloudflare-dns.md` (private — this is operator-specific; not in the public docs site).
- [ ] AE.8 GitHub Actions self-hosted runner. Install official `actions/runner` under `~recon-demo/runner/`. Register with label `mac-mini-demo`; tied to a narrow GitHub App or PAT scoped to read+actions:write on this repo only. Run as launchd service `io.hotchkiss.recon-demo.runner.plist`. New workflow `.github/workflows/demo-publish.yml` triggers on `release` published events, picks up `runs-on: [self-hosted, mac-mini-demo]`, runs the refresh script directly (skipping the wait for the 03:00 cron — release-day refresh is immediate).
