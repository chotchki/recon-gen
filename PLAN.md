...
- [ ] AM.1 - Convert editor screens (create / edit / list / singleton / read-card) to raw Tailwind utilities per L2. Drop the `editor.css` rules. Update `@source` to include `_studio_editor_routes.py`. **Fold in the AI.2.e part-2 stretch** — richer subtype-aware per-field requirement hints beyond the existing `*` markers + entity intro (e.g. a subtype requirements banner on the rail form).
