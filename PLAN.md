...
- [ ] AI.11 - **Fuzzer limitation surfaced by AI.2.d.2 piece 3 (2026-05-25)**: `tests/l2/fuzz.py::random_l2_yaml` deliberately declines to generate Variable-direction single-leg rails (per its own docstring: "need template + last-leg-posted semantics — overhead vs payoff is poor for an in-process generator"). This blind spot LET the fuzz axis pass on the HTTP transport without surfacing the AI.10 UI gap. Real customer L2s commonly carry Variable-direction rails (every settlement / disbursement TT-with-Variable-closing-leg). Extend the fuzzer to generate Variable rails alongside TTs that close them — with the proper `leg_rail_xor_groups` partitioning when >1 Variable per TT. Will increase fuzz-surface noise but earn its keep on the same shape AI.10 needs.
