...
- [ ] AI.8 - **UI gap surfaced by AI.2.d.2 (2026-05-25)**: editor sub-pages (list / create / edit / read-card) don't render a link back to the studio home `/`. Real operators must use the browser Back button or re-type the URL — neither is an editor-affordance. Add a consistent home-link in the studio chrome (breadcrumb, header logo-link, or "Back to studio" button) that lands on every sub-page. AI.2.d.2's `goto_home()` verb tried to click `a[href="/"]` and hit Playwright timeout — that's the regression class that AM Tailwind sweep could mask if AI.2.d.2 hadn't surfaced it.
...
- [ ] AI.10 - **UI form-pairing gap surfaced by AI.2.d.2 piece 3 (2026-05-25)**: `partial_xor_groups` (BB.3 wave logic) needs to land in the SAME composite POST as the rail-create when attaching a Variable-direction rail to a TT reconciler that already has other Variable rails (otherwise validator C1 fires). The HTTP driver sneaks the `leg_rail_xor_groups_*` fields into the POST body alongside `reconciler_kind` / `reconciler_name`; the BROWSER can't sneak — only fields rendered on the form get submitted. The rail-create form doesn't render xor_groups inputs (xor_groups is `edit_only` on the TT). Same shape as BB.0's S3/C3 bilateral form-pairing: when the operator attaches a Variable rail to a TT with existing Variables, the form should pair the xor_groups choice. AI.2.d.2 piece 3 first-pass scopes around this (uses a fixture without multiple-Variable-in-one-TT); spec_example needs the form-pairing UI to land.
...
- [ ] AI.13 - **BB.2 sub-form gap: aggregator-rail two-leg subtype missing `expected_net` (surfaced by AI.2.d.2 piece 3, 2026-05-25)**: the BB.2 reconciler create-new sub-form's aggregator-rail two-leg block only renders `subtype + cadence + source_role + destination_role`. A standalone two-leg aggregator-rail still needs `expected_net` set (validator: "standalone two-leg rail … MUST declare expected_net (typically 0)"); the form doesn't expose the field, so an operator creating a two-leg aggregator via BB.2 hits a hard wall. Add `expected_net` to the BB.2 aggregator-two-leg sub-block, OR document that aggregator-two-leg can't be created via BB.2 (forcing operator to create the rail FIRST, then attach). Per the BB.0 form-pairing principle: form should expose every field the validator requires. Same shape as AI.10 (BB.2 form gaps).
