...
- [ ] **X.4.c.5** — Coverage-tint overlay: `coverage_for(connection, prefix, l2_instance)` data fetcher (binary presence per L2 primitive, row-count on hover).
  ...
  - [ ] **X.4.c.5.d** — Chrome surface: "Coverage" toggle in `_render_diagram_page`'s chrome bar. Off by default (clean diagram). When on, `diagram.js` fetches `/diagram/coverage` once, stamps `data-presence="yes|no"` + `data-row-count="N"` per `g.node[data-id]`, applies a CSS class to the SVG root (`.coverage-on`) so the per-presence tint rules activate.
  - [ ] **X.4.c.5.e** — Hover surface: `<title>` element injected on each tinted node carries `"<label> · <count> rows"` so the browser's native hover tooltip shows the count. (Cheaper than a custom tooltip widget; works inside the SVG.)
  - [ ] **X.4.c.5.f** — Tests: unit for `coverage_for` against SQLite + a hand-built `<prefix>_transactions`; browser e2e under `tests/e2e/test_studio_diagram_coverage.py` toggling the checkbox + asserting `data-presence` lands on the role node for an empty role.
