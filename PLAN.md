...
- [ ] AI.15 - **BB.2 create-new sub-form misses optional rail fields (surfaced by AI.2.d.2 piece 3 end-to-end 2026-05-25)**. After AI.10 + AI.13 + piece-4's `_edit` verb, the full `create_l2` walk via real browser runs end-to-end — but the AI.4 structural diff still fails because the BB.2 create-new sub-form for aggregator-rail reconcilers only renders validator-required minimums (subtype + cadence + role + direction + expected_net + origin). Optional rail fields the operator might want set on the new aggregator (`metadata_keys`, `amount_typical_range`, `firings_typical_per_period`, `description`, `posted_requirements`, `max_pending_age` / `max_unbundled_age`, `bundles_activity`) aren't rendered, so a create-new'd reconciler can never match a reference that sets those. Two fix shapes: (a) widen BB.2 sub-form to render every rail field (busy sub-form but completeness wins), OR (b) post-create wave in the browser driver that EDITs each create-new'd reconciler to fill any reference fields the sub-form missed (driver-only fix; doesn't touch UI). (b) is the contained piece-4-extension answer; (a) is the better operator UX. Decide post-AM.
- [ ] AI.14 - **Eliminate free-form textareas where structured pickers fit better** (logged 2026-05-25; defer until AI + AM ship to establish a baseline first). AI.10 landed an MVP `leg_rail_xor_groups_text` textarea for xor_groups partitioning — operator types comma-separated rail names per group. Functional but UX-poor: typo-prone, no autocomplete, no visual hint at which rails are eligible, no validation feedback until submit. Real operator UX is a per-rail multi-select-group widget (similar to the existing `multi_select_groups` field-kind in `_FIELD_SPECS_BY_KIND`'s TT.leg_rail_xor_groups — but as a rail-create-form affordance rather than TT-edit). Also audit other textarea fields (`metadata_keys`, `posted_requirements`, `metadata_value_examples`) for similar improvements. Same shape as AI.8's no-link-in-chrome — feature works but operator ergonomics need a richer widget. Land after AM Tailwind sweep so the visual chrome is unified first.
