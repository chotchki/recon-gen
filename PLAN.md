...
- [ ] AE.10 Docs + runbook. New `docs/operations/mac-mini-demo-host.md` covering: install steps (referencing AE.1-AE.8), the launchd service lifecycle (`launchctl list io.hotchkiss.recon-demo.*`), how to troubleshoot a stuck refresh, how to rotate the Cloudflare tunnel credentials, how to roll back to a prior wheel. Mostly operator-private — `docs/operations/` is gitignored from the public docs site build, lives in the repo but not the rendered site.
