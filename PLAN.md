...
- [ ] AP.0 AP.0 Spike: the as_of frame (D1) on one surface
  ...
  - [ ] AP.2 AP.2 Spike: invariant spine (D6) — invariant owns detect + scenario_for(shape,selector); settle generator shape: state-step/simulation (non-violating = clean run, violation = perturbation) + Local vs Populational. See audit §5 AP.3-extension findings.
