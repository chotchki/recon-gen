...
- **QS silent-fail mode** — datasets healthy + describe-cleanly, every visual on every sheet shows the spinner forever. See CLAUDE.md → Operational Footguns for the diagnostic ladder.
  - [ ] Inbox.0 Inbox
    ...
    - [ ] Inbox.2 AE.2.b.chrome — UI cosmetics for studio `--demo-mode`: (1) `_render_home_page` + `_render_data_page` + `_render_diagram_page` accept `demo_mode: bool` and emit a top-of-page banner "Read-only demo — mutations disabled" with a link to the operator's docs. (2) Deploy-changes button conditionally rendered (hidden when demo_mode). (3) L2 editor save / create / delete buttons in the editor card chrome conditionally rendered. Browser smoke: assert banner present, Deploy button absent in DOM. Defer rationale: the route-level lockdown is load-bearing safety; chrome cosmetics are friendlier UX but not a correctness gate. Rehomed from Phase AE on 2026-05-19 when AE archived.
    - [ ] Inbox.3 README.md — next update should add live demo links: `https://recon-gen-spec.hotchkiss.io/` (dashboards) and `https://recon-gen-sasquatch.hotchkiss.io/` (studio --demo-mode). User-flagged 2026-05-19: "seeing is proof it works" — Phase AE shipped public demos that should land on the README's first scroll.
  - [ ] Inbox.4 CI coverage aggregator reports "No data to combine" → empty report artifact. **Root cause:** `actions/upload-artifact@v7` defaults `include-hidden-files: false`; `.coverage.*` files (leading dot = hidden by macOS/Linux convention) get excluded from the artifact payload. Aggregator job downloads empty artifacts → `coverage combine` finds nothing. **Fix:** add `include-hidden-files: true` to every upload-artifact step that captures `.coverage.*` data files in `.github/workflows/e2e.yml` + `.github/workflows/release.yml`. ~6 step diffs. No code change needed; pure workflow yaml fix. Surfaced 2026-05-20 during AB.7.1a release verify after CI told us 81% but the aggregator silently lost every per-leg coverage measurement. Verify with: re-run CI, check that the `Combine coverage data files` step logs "Combined data file ...; 6 source files combined" (or similar) instead of "No data to combine".
  - [ ] Inbox.5 Rich text rendering broken on multiple L1 dashboard sheets (App2 renderer). User-observed 2026-05-19/20 against `recon-gen-sasquatch.hotchkiss.io`:
    ...
    Both likely share a root cause (rich text box rendering pathway in `common/html/` for the L1 dashboard's HTMX render path). Possible suspects: (a) sheet description / SheetTextBox renderer mis-escaping the rich-text XML when emitting HTML; (b) bottom-of-page placement triggers a CSS/layout collapse that hides content; (c) a v11.6.x regression in the App2 renderer's handling of multi-paragraph text blocks. **Investigation steps:** (1) load each URL in a browser and inspect DOM for the affected rich-text box → confirm it's "wrong HTML" vs "missing HTML"; (2) compare to QuickSight rendering of the same sheet (if both render correctly there, the bug is App2-specific); (3) bisect via git log on `common/html/` to find the introducing commit. **Severity:** integrator-visible — demo viewers see broken rendering on the public sasquatch demo. Fix priority is high once root cause is identified.
