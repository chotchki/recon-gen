...
- [ ] **X.1.b — Re-enable + diagnose L2FT cascade test.** Re-skipped May 2026 after an experiment cycle. Findings:
...
- [ ] **X.1.g — Convert L2FT CategoryFilter dropdowns to `ParameterDropdown(StaticValues)` shape.** Bigger restructure than the X.1.b experiment attempted. For each of the 7 `select_all_options="FILTER_ALL_VALUES"` dropdowns across L2FT (Rails: rail_name / status / bundle_status; Chains: parent_chain_name / completion_status; TT: template_name / completion_status), replace the `CategoryFilter` + `add_filter_dropdown` shape with a `StringParam` + parameter-bound `CategoryFilter` + `add_parameter_dropdown(StaticValues=[...])`. Same pattern the Metadata Key dropdown already uses successfully. Eliminates the lazy `tenK-sample-values-V2` fetch path entirely. Helpers already exist in `datasets.py`: `declared_rail_names`, `transaction_status_values`, `bundle_status_values`. Need to add: `declared_chain_names`, `completion_status_values`, `declared_template_names` (the last already exists). Side benefit: lets us drop the indexes added in v8.5.6 / v8.6.8 specifically to support those dropdown queries. After this lands, re-enable the L2FT cascade test (X.1.b) and verify the empty-table failure is gone.
