...
- [ ] **AB.1.2 — `primitives.py`: new `LimitDirection` enum + `direction: LimitDirection = LimitDirection.OUTBOUND` on LimitSchedule.** Per AB.1.0 lock — separate enum from `Amount.Direction`. Backward-compatible default.
- [ ] **AB.1.3 — Loader + serializer + validator.** Loader accepts `direction: Inbound` / `Outbound` / unset (default OUTBOUND). Validator: U5 / R7 unchanged (parent_role + rail uniqueness still holds across direction — but reconsider: does AB.1 allow per-(parent_role, rail) BOTH an Outbound + Inbound cap simultaneously? If yes, U5 needs to broaden to `(parent_role, rail, direction)` uniqueness). New rule: `direction` value must be one of the two `LimitDirection` literals.
- [ ] **AB.1.4 — `schema.py`: `<prefix>_limit_breach` matview UNION ALL two direction-branches.** Per AB.1.0 lock. `_render_limit_breach_cases(instance, *, p, dialect, direction)` filters the inlined CASE branches to LimitSchedules matching `direction`. Matview's two SELECTs each filter `tx.amount_direction = 'Debit'` (OUTBOUND branch) or `'Credit'` (INBOUND branch), with a literal `'Outbound'`/`'Inbound'` as a new `direction` column in the projection. SUM-window unchanged; per-day aggregation primitive doesn't shift.
...
- [ ] **AB.1.5.fuzz — `tests/l2/fuzz.py`: emit `direction` on synthesized LimitSchedules.** `random_l2_yaml(seed)` already emits LimitSchedules (default-Outbound); extend the synthesizer to randomly pick `direction: Inbound` for ~30% of them (deterministic per seed) so the fuzz matrix cells exercise both directions. The (parent, rail, direction) uniqueness constraint forces the synth to dedupe across direction too — re-emit logic mirrors existing (parent, rail) dedupe.
- [ ] **AB.1.5.spec — `tests/l2/spec_example.yaml`: add 1 Inbound LimitSchedule.** The minimal fixture every dialect-snapshot test reads MUST exercise the new code path. Choose any pre-existing (parent_role, rail) pair from the fixture's rails and add a sibling LimitSchedule entry with `direction: Inbound`. Re-lock the spec_example seeds (`recon-gen data lock -c run/config.{postgres,oracle,sqlite}.yaml --l2 tests/l2/spec_example.yaml`) — they'll grow inbound-breach plant rows now.
