...
- [ ] **Y.2.gate.b — Design the runner.** Spike (b.0) returned. Substrate locked: pytest-as-orchestrator + ~400 LOC Python wrapper at `quicksight_gen/_dev/runner.py`, surfaced via `./run_tests.sh` bash shim. Each design item below tracks against that substrate.
    ...
    - [ ] **Y.2.gate.b.12 — Pytest version + JSON-hook plugin confirmation** (open follow-up from spike). The locked design uses `pytest_runtest_protocol` hook to emit per-test timings to JSON. Confirm: (a) the project's pinned pytest version supports the hook with the right args; (b) no new plugin dependency is needed (custom conftest hook is sufficient — `pytest-json-report` exists but adds a dep). Output: 1-paragraph note in the spike doc + dep verdict.
    - [ ] **Y.2.gate.b.13 — `_dev/` package shape lock-in.** Decide between `quicksight_gen/_dev/runner.py` (under the wheel package, gated out of customer install via `pyproject.toml::tool.setuptools.packages.find::exclude`) vs `tests/_runner/` (in the repo but not in the wheel at all). Lean toward `quicksight_gen/_dev/` so it's importable from conftest cleanly; verify the wheel-exclude mechanism actually keeps it out of customer installs (CI: `pip install <wheel>; python -c "import quicksight_gen._dev"` should fail).
    - [ ] **Y.2.gate.b.14 — Bash entry-point shim minimum viable shape.** Spec out the ~10-line `./run_tests.sh`: shebang, `set -euo pipefail`, "venv exists?" check (`[ -f .venv/bin/python ] || { echo "run uv sync first" >&2; exit 1; }`), `exec .venv/bin/python -m quicksight_gen._dev.runner "$@"`. No arg parsing in bash — orchestrator owns it. Document in the runner module's top-level docstring.
