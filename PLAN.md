...
- [ ] **Y.2.gate.c — Implement the runner.** Substrate: pytest + `quicksight_gen/_dev/runner.py` (~400 LOC budget) + `./run_tests.sh` bash shim (locked by `Y.2.gate.b.0` spike). Each sub-task carries the module / function / hook ownership.
    ...
    - [ ] **Y.2.gate.c.6 — Within-layer parallelism** (split 2026-05-07 into c.6.parallel done + c.6.async still open).
        ...
        - [ ] **Y.2.gate.c.6.xdist-safety — fix fuzz tests for parallel collection.** `tests/json/test_l2_flow_tracing_matrix.py` parametrizes test ids with random fuzz seeds (`fuzz-seed-<random>`); each xdist worker rolls its own seed → workers diverge on collected tests → `Different tests were collected between gw0 and gw1` error → `pytest -n N` exits 1. Fix: pin the fuzz seed across workers (use `QS_GEN_FUZZ_SEED` env, set deterministically by the runner before pytest spawns; conftest sessionstart reads it). Keeps audit §7.11's "default = 1 random seed per run" semantics — still random per RUNNER INVOCATION, just shared across workers within an invocation.
