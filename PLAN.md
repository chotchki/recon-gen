...
- [ ] BC.14 - **Pre-existing Oracle data-apply ORA-01843 (`invalid month`).** Surfaced behind BC.12's ORA-32368 wall — Oracle's NLS_DATE_FORMAT defaults differ from what the spine's seed-emit TIMESTAMP literals assume. Fails at stmt #2368 of `data apply` (sp_or_lo). Out of BC.12 scope (data-apply path, not schema-apply); needs its own spike on Oracle NLS_DATE_FORMAT handling in the seed INSERT emitter (`common/l2/seed.py`? `common/db.py::batch_oracle_inserts`?). **Diagnostic-first sub-tasks**:
  - [ ] BC.14.1 - Reproduce + capture the failing INSERT row + the actual literal that ORA-01843s. Hypothesis: the seed emits `TO_TIMESTAMP('2026-05-24 12:00:00')` without an explicit format mask; Oracle's NLS_DATE_FORMAT defaults to `DD-MON-RR` which doesn't match.
  - [ ] BC.14.2 - Fix at the right seam — either set session NLS_DATE_FORMAT explicitly at `connect_demo_db` time (one-line workaround) OR emit explicit format masks in the seed (`TO_TIMESTAMP('2026-05-24 12:00:00', 'YYYY-MM-DD HH24:MI:SS')` — more robust, propagates to operator-piped SQL too). Lean second.
  - [ ] BC.14.3 - Verify integration-oracle CI job goes green end-to-end (was schema-apply red pre-BC.12; will be data-apply red post-BC.12 until BC.14 lands).
