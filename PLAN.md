...
- [ ] **X.1.k — Unify the seed-hash lock surface; key the CLI off config.yaml, not the L2.** Today the hash lock has four distinct sites and one CLI that updates one of them: (i) `seed_hash.postgres` in each L2 YAML, (ii) `seed_hash.oracle` in each L2 YAML, (iii) the Python full-seed hash constants in `tests/data/test_l2_baseline_seed.py`, (iv) `_BROAD_MODE_HASHES` in `tests/data/test_l2_seed_contract.py`. `quicksight-gen data hash --lock` only writes (i) because dialect was hardcoded to postgres. A partial re-lock during X.1.i ⇒ CI red on every commit between 2026-05-04 21:58 and the fix-CI commit (`ff40c8e` … `3b113ed`); the docs-templating commit `ff40c8e` added 3 unrelated failures on top, masking that the lock surface was the real problem.
  ...
  **Storage shape — pick one:** (a) keep the four-site layout but make `--lock` re-write all four for the resolved dialect (postgres OR oracle in one call) and add an `--all-dialects` shortcut that reads both configs from a sibling lookup; (b) collapse sites (iii) + (iv) into the L2's `seed_hash:` block as `full_seed.{postgres,oracle}` + `broad.{postgres,oracle}` keys, drop the Python constants, have the tests load the YAML — single source of truth per dialect, lock just regenerates the YAML. Lean toward (b): fewer files to align, failure mode collapses to "did you re-run `--lock` against both configs?".

  **Gate:** add `data hash --check --strict` (resolves dialect from config, asserts every hash-class for that dialect matches) as a CI step before the unit suite, run once per `(config.postgres.yaml, config.oracle.yaml)` so a missed re-lock fails with one line, not 7+ scattered assertions.
