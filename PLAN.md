...
## Phase AO — v11.9.4 cold-read warm-pass dashboard fixes *(source: `docs/audits/v11_9_4_feedback.md` — 4 context-isolated cold-judge passes; per-finding agreement N/4 + triage routing live in that file. Data-correctness AND presentation both in scope.)*
...
- [ ] AO.2 (feedback #1 · BLOCKER 3/4 · KEYSTONE) Daily Statement summary KPIs don't compute / don't tie to the detail: Debits & Credits read 0 on populated accounts while the Posted-Money-Records table is full; Drift KPI stuck at 0 where a recompute gives −992,605; the statement's drift=0 contradicts the Drift sheet. Hypothesis: the statement isn't pinned to a single `business_day`, so the per-day KPI tiles and the all-days detail table disagree. Investigate the daily_statement KPI datasets + balance-date param pinning + the `today_flows` join in `_daily_statement_summary`. **Top priority — the two numbers you open the dashboard for don't compute.**
...
- [ ] AO.6 (feedback #6 · MAJOR 1/4) L2 Exceptions "count" overloaded — "Open L2 Violations = 39" directly above a detail row count of 410. Disambiguate the labels / scope (distinct violations vs leg rows).
...
- [ ] AO.8 (feedback #8 · MAJOR 2/4) Chart presentation: per-rail bar-chart x-axis is an unreadable `rail_name` label smear (both exec "Period Total by Type" charts), and "stacked by type" charts show no stacking/legend. Rotate/truncate/wrap long categorical labels; apply the stacking visual-property + legend.
- [ ] AO.9 (feedback #9 · MINOR multi) Empty-state clarity + Anomalies flag-logic: Volume Anomalies KPI "0 flagged" contradicts its own populated σ-distribution chart — data HAS 129 anomalies at the default 2σ (67 at 3σ, max z=25.3), so likely the App2 sigma SLIDER default (2) isn't bound on initial load (sibling to the v11.9.4 picker bug) and/or scope wording; confirm by nudging the slider. Plus: as-of stamps (Limit Breach "clean vs didn't run"), dollar-exposure context (Unbundled 802 / Supersession), clipped y-axis ("0,000,000"), context for the net "money moved" figure.
