...
- [ ] AY.4 - Reroute production seed (build_full_seed_sql + emit_full_seed) through ScenarioContext.compose
  ...
  - [ ] AY.4.g - Chain-completion shim for plants whose emit fires a chain-parent without its child legs. OLD path: `_emit_plant_chain_completion` co-planted the child leg per chain entry, suppressing `multi_xor_violation` false-positives. Spine generators omit this (documented in plant_adapter.py header). Surfaces as `test_multi_xor_violation_holds_only_intended_plants[sasquatch_pr]` false-positive incidentals (3 rows). Options: (a) add `ChainCompletionGenerator` to the adapter for every plant that fires a chain parent, or (b) widen the matview's plant_id filter to exclude `tr-xor-*` / `xfer-limit-breach-*` prefixes. Pick at AY.4.g spike. **Blocks AY.5.**
