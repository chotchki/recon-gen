...
### Decisions to lock before AE.1 fires (open for AE.0)

- **Cloudflare proxy mode**: Cloudflare Tunnel (cloudflared) is the security-correct default — no inbound port forward, no public IP exposure, hostnames bind directly to the tunnel. Orange Cloud + port-forward is simpler but punches a hole in the home network. **Tunnel preferred**; revisit only if cloudflared adds operational friction.
- **Sandbox shape**: `sandbox-exec` with a profile that allows network on the configured ports, read-only access to the venv + L2 yaml, read-write access to the per-instance SQLite db + log dir, denies everything else. macOS deprecated `chroot` in favor of this. (Docker is option B if `sandbox-exec` profile churn is high — `colima` provides Docker on Mac mini cheaply.)
- **Refresh cadence + execution shape**: nightly at 03:00 America/New_York (off-peak both for the host and for any human viewer). Refresh = `recon-gen schema apply --execute` + `data apply --execute` + `data refresh --execute` against the SQLite db file. Atomic swap: build new db file in `~/recon-demo/<instance>/next.sqlite3`, fsync, mv to `current.sqlite3`, server process holds the path via symlink and re-opens on SIGHUP. (Or restart the server — simpler, brief 5s outage acceptable for demo.)
- **Server entry**: `recon-gen dashboards -c ~/recon-demo/<instance>/config.yaml --l2 ~/recon-demo/<instance>/l2.yaml --port <8401|8402>`. No `studio` (editor is integrator-facing; this is read-only demo). Bound to `127.0.0.1` only — cloudflared bridges to public; nothing else on the LAN should reach these ports.
- **Update flow**: launchd timer fires `pip install --upgrade recon-gen` from PyPI inside the dedicated user's venv before refresh. CI doesn't push to the runner; the runner pulls. Pinning to a specific release tag is an option for stability (env var `RECON_GEN_PIN_VERSION`).
- **GitHub runner role**: registered with label `mac-mini-demo`; queued only for `.github/workflows/demo-publish.yml` (new). Not a generic build runner — narrow surface, narrow blast radius.

- [ ] AE.0 Locks (6 decisions before AE.1 fires, 2026-05-19). Confirm Tunnel vs Orange Cloud, sandbox-exec vs Docker, refresh cadence + atomic swap shape, server entry + bind address, update-pull flow, runner scope.
- [ ] AE.1 macOS user + filesystem layout. Create `recon-demo` user (no admin, no GUI login). `~recon-demo/` layout: `venv/` (per-user uv venv), `spec_example/{config.yaml, l2.yaml, current.sqlite3, next.sqlite3, logs/}`, `sasquatch_pr/{...}`, `runner/` (GitHub runner install), `tunnel/{cloudflared.yml, credentials.json}`. Install path includes `~recon-demo/venv/bin` only. Owns nothing outside `~recon-demo/`. Document via `docs/operations/mac-mini-host-setup.md`.
- [ ] AE.2 Sandbox profile. New `deploy/sandbox/recon-demo.sb` per-instance sandbox-exec profile: allow `network-bind` on `127.0.0.1:8401` (spec) or `127.0.0.1:8402` (sasquatch) per-process; allow file-read on `~recon-demo/<instance>/{config.yaml,l2.yaml}` + venv + system Python stdlib; allow file-write on `~recon-demo/<instance>/{current.sqlite3,logs/}`; deny everything else. Test the profile by running `sandbox-exec -f recon-demo.sb /bin/sh` and trying disallowed ops (should fail). Cross-reference: `man sandbox-exec`, the Mozilla / Chromium sandbox profiles as known-working examples.
...
- [ ] AE.4 launchd: dashboards server jobs. Two plist files in `~recon-demo/Library/LaunchAgents/`: `io.hotchkiss.recon-demo.spec.plist` + `io.hotchkiss.recon-demo.sasquatch.plist`. Each runs `sandbox-exec -f recon-demo.sb -- venv/bin/recon-gen dashboards -c <instance>/config.yaml --l2 <instance>/l2.yaml --port <port> --bind 127.0.0.1`. `KeepAlive=true`, `RunAtLoad=true`, log stdout/stderr to `<instance>/logs/server.{out,err}.log` (rotated weekly via launchd's built-in log facilities).
