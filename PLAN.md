...
- [x] **Y.2.f — L1 universal date range pushdown — DONE (2026-05-10) via Option B: dual-SQL.** Same `app2_date_filter` dual-SQL pattern as Y.2.h, applied to all 8 L1 datasets that the analysis-level `TimeRangeFilter` family scopes (drift, ledger_drift, drift_timeline ×2, overdraft, limit_breach, todays_exceptions, transactions). Each builder converts its SQL to a `sql_template` with a `{date_filter}` slot; `qs_sql = template.format(date_filter="")` (zero QS behavior change — the analysis-level FG continues to do the date filtering); `app2_sql = template.format(date_filter=app2_date_filter("<col>", cfg.dialect))` so when L1 lands on App2 the date binds reach the SQL automatically. Ducks the Y.1.k `RollingDate`-vs-`StaticValues` wart entirely — QS never sees a dataset DateTime param, so there's no opening-view shift. Stuck-pending / stuck-unbundled / supersession / daily-statement intentionally untouched (current-state matviews; date filter would diverge from the audit PDF, breaking U.8.b's three-way agreement). Drift/ledger_drift timelines insert the `{date_filter}` slot BEFORE the `GROUP BY` so the AND-clause stays in the WHERE.
    - [ ] **Y.2.f.2 — X.2.j 4-way agreement: validate L1 date-filter parity (User flagged 2026-05-10).** The dual-SQL approach has a known parity gap to verify at X.2.j time: QS's analysis-level `TimeRangeFilter` operates on the FilterGroup's `time_granularity="DAY"` semantic (which truncates the column at filter-eval time), while `app2_date_filter`'s SQL `column >= TO_DATE('YYYY-MM-DD', ...) AND column <= TO_DATE('YYYY-MM-DD', ...)` compares against midnight of those dates. For a `posting TIMESTAMP` value of `2026-05-10 14:32:01` and a `:date_to = '2026-05-10'` bind, the SQL clause's `<=` would EXCLUDE it (14:32 > 00:00). QS's DAY-granularity filter likely INCLUDES it (truncates posting to date first). When wiring L1 into App2 (X.2.j candidate) verify the row counts agree across QS / App2 / SQL / Audit PDF for the same date window; if they don't, either widen `app2_date_filter` to `< (date_to + 1 day)` or apply a `DATE_TRUNC` to the column on the App2 side. Parity gap might also bite Executives + Investigation app2 datasets that already use `app2_date_filter` (they were never explicitly verified against QS's filter-eval). Same conversation also revisit: the `posting`-column day-granularity-inclusivity question + the QS-DateTime-param-substitution shape if we ever pivot to Option A.
