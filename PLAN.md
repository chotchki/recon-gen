...
**Confirm-on-AWS:**
- [x] AO.0 - v11.9.4 cold-read warm-pass dashboard fixes *(source: `docs/audits/v11_9_4_feedback.md` — 4 context-isolated cold-judge passes; per-finding agreement N/4 + triage routing live in that file. Data-correctness AND presentation both in scope.)*
  - **DONE 2026-05-24** — all 10 cold-read items closed. Released v11.17.0 (AO.L + AO.1.impl + AO.4 + AO.7); follow-on patches AO.5, AO.L.gate, AO.9, AO.S, AO.C1/C2 + Oracle CI fix on `main` post-v11.17.0 (queued for v11.18.0 tag). Phase AO sweep-to-archive pending the next cold-read pass.
- [x] AO.1 - (ROOT fix) Store money as integer cents, translate back at the edges (user call 2026-05-21 — materiality band rejected as a band-aid). **Spike done:** cents-space drift = 2 genuine vs 116 float-noisy on sasquatch_pr SQLite (≈ PG's exact NUMERIC → restores cross-dialect agreement); blast radius = 5 money columns, ~45 `currency=True` display sites (unchanged), money math concentrated in a few matviews. Design: money MATH in integer cents, translate cents→dollars at ONE chokepoint (`Current*` / matview projection). Footguns: ingest `×100` round-to-int at the `etl_hook`; literals flip to cents; divisions cast to NUMERIC; a missed `÷100` = silent 100× error.
  ...
  - [x] AO.1.impl - Per lock: money cols → BIGINT cents; matview math in integer cents; `Current*`/matview projection → dollars; seed emits cents + `etl_hook` `×100`; literals → cents; re-lock seeds; SQLite structural drift count 116→2; full suite + 4-way green; commit + patch release.
    - **DONE — shipped in v11.17.0** (merge commit `3bf9acfc` on main, tag `v11.17.0` pushed). Helper-template chokepoint + `Cents` newtype design landed via 5-slice agent fan-out + integration commit. Drift target hit: `sq_drift` 675→2, `sq_ledger_drift` 37→1 (genuine plants only). All 6 sub-slices done:
    - [x] AO.1.impl.1 foundation — `Cents` type + SQL helper + 3 cols → BIGINT + insert helpers convert at boundary + spine detectors wrap reads. Commit `05b4872a` (cherry-picked as `4abbd854` on integration).
    - [x] AO.1.impl.2 dataset SQL — 4 apps' datasets wrap money projections with `cents_to_dollars_sql()`. Commits `857e98b2` + `c6d3b8d9` (cherry-picked as `95453eeb` + `f1d7867f`).
    - [x] AO.1.impl.3 audit PDF — cursor-boundary `Cents.from_db(...).to_dollars()` at 12 read sites; provenance hashes cents (verifier symmetric). Commit `2c49c638` (cherry-picked as `55499e7f`).
    - [x] AO.1.impl.4 Studio — data-shaping panel + trainer + editor read sites convert at boundary; live Studio smoke. Commit `0ca86c4b` (cherry-picked as `2feb1587`).
    - [x] AO.1.impl.5 ETL examples + docs — `etl_examples.py` flipped to integer cents + new Pattern 11 `Cents.from_dollars(...).value` wrapper; `etl_hook` cfg docstring updated; `docs/Schema_v6.md` 3 column-types flipped + new "Money is integer cents" section; walkthroughs updated. Commit `06219e02` (cherry-picked as `2ea5184c`).
    - [x] AO.1.impl.6 integration — 5 slices cherry-picked onto `ao-1-money-cents`; full suite + magnitude split (AO.4); v11.17.0 release notes + tag; merged to main via `3bf9acfc`.
...
- [x] AO.4 - (feedback #3 · MAJOR 3/4) Today's Exceptions count integrity: KPI 828 vs footer "1–50 of 935"; not sorted by magnitude (multi-million `ledger_drift` buried under magnitude-0 rows); rows span multiple days despite "today." Fix the count source + default sort (magnitude desc) + "today" scoping.
  - **DONE — shipped in v11.17.0 (commit `e10115cb`).** Matview split: `magnitude_amount` (BIGINT cents, money branches) + `magnitude_count` (INTEGER, transfer-keyed cardinality branches). Exception Detail shows both side-by-side; sort defaults to amount DESC. Pre-AO.1 magnitude-0 float-noise rows gone structurally via integer cents. **NOT DONE in this pass** (queued as AO.4.followup if operator wants): KPI vs footer count-source mismatch (KPI counts non-null account_id; transfer-keyed branches have NULL account_id → 107-row diff explains the cold-read's 828 vs 935); "today" sheet name vs multi-day transfer-keyed/stuck rows (matview comments explicitly include all days for those branches — sheet name lies).
- [x] AO.5 - (feedback #5 · MAJOR 2/4) Executives "Average Daily Volume" ≈67× off vs total ÷ active-days — avg calc bug (wrong denominator / window).
  - **DONE — code on `main` (post-v11.17.0), pending operator visual + tag.** KPI was `ds_txn["transfer_count"].average(...)` on the per-(date, rail) dataset; with Sasquatch's ~30 rails firing per day, AVG over (date × rail) rows ran ~25× too small (cold-read reported ~67× — depends which day's rail-count the judge sampled). Fix: new `exec-transaction-daily-ds` dataset that rolls per_transfer to one row per active day; KPI consumes it so AVG denominator = days-with-activity. Live spot-check on sasquatch_pr/SQLite: old=22.97, new=581.98 = total(38411)/active_days(66) exactly. Tests updated for the dataset count + index shifts (json/test_executives.py). Time-axis "outage + gap" framing → AO.S.2.
...
- [x] AO.9 - (feedback #9 · MINOR multi) Empty-state clarity + Anomalies flag-logic. **Anomalies "0 flagged" → DONE via AO.R.4** (slider int-binding; live 2σ→129 / 3σ→67). **Still open (real dashboard *content*, both renderers — not a render gap):** as-of stamps (Limit Breach "clean vs didn't run"), dollar-exposure context (Unbundled 802 / Supersession), clipped y-axis ("0,000,000"), context for the net "money moved" figure.
  - **DONE — code on `main` (post-AO.L.gate), pending operator visual + tag.** Four sub-items:
    1. **Empty-state clarity**: tightened KPI subtitles on Limit Breach (L1), Recipient Fanout + Volume Anomalies (Investigation) to explain what "zero" means + point at App Info → matview-status table for the "stale matview" disambiguation. Matview-status data already exists per app's App Info sheet; the subtitle change directs the analyst there without adding a cross-sheet KPI.
    2. **$ exposure**: added paired/extra `SUM(amount).currency` KPI alongside the row-count KPIs on Unbundled Aging (HALF/HALF row) + Supersession Audit (THIRD/THIRD/THIRD row — count, $, no-reason). Cold-read judge now sees both how many AND how much.
    3. **Clipped y-axis**: App2 BarChart + LineChart's hardcoded `margin.left = 64` clipped `$10,000,000`-class labels to `0,000,000`. Both now scale margin.left from the data magnitude + currency-format prefix (`prefix + digits * 8 + commas * 3 + 12` px, min 64). Tested via biome lint + suite green.
    4. **Money-moved net context**: Net Money Moved KPI subtitle expanded — explains "expected near zero on a balanced book" + the offsetting-leg semantics + when large +/− values are explainable. Comparison to Gross is now explicit.
...
- [x] AO.S - seed-residual annotation cluster
  - [x] AO.S.1 - (feedback #3/#4) Magnitude-0 exception rows + pool `ledger_drift` in the millions = the known plant-residual / independent-leg fan-out modeling. AO.1's integer-cents math clears the float-noise portion; decide fix-seed-to-reduce-noise vs annotate-as-intended for the rest. NOTE: feedback #4's "child postings don't roll up to parent / no working drill" IS a real dashboard concern (the dead-end drill depends on AO.2).
    - **DONE (post-AO.9, code on `main`).** Verified post-AO.1.impl + AO.L on sasquatch_pr/SQLite: `sq_drift` magnitude-0 rows = 0 (integer cents killed float noise); `sq_ledger_drift` magnitude-0 rows = 0 (AO.L cumulative-vs-delta fix); `sq_todays_exceptions.magnitude_amount = 0` rows = 0. The one remaining `sq_ledger_drift` row is the deliberate **DDAControl plant** (-$2.8M, the L1 SPEC cross-boundary example). Annotated the "Parent Accounts in Drift" KPI subtitle so analysts know that persistent ~$2.8M is intentional in the bundled demo (real deploys should see 0). Feedback #4's drill concern auto-closed via AO.2 piece-1 (date pushdown so the Daily Statement opens populated).
  - [x] AO.S.2 - (feedback #5) Exec time-axis multi-week empty stretch + weekend gaps = the deliberate short seed window — annotate the chart so it doesn't read as an outage.
    - **DONE (post-AO.9, code on `main`).** Added the note to both Exec time-series charts ("Daily Transaction Count by Type" + "Daily Gross Dollars Moved by Type"): apparent multi-week empty stretches + weekend gaps reflect the demo's 90-day seed window + non-business-day cadence, not data outages. Real-deploy ETL with full history won't show them.
...
- [x] AO.C1 - (feedback #10) App Info shows `dialect: sqlite` / dev prefix — fine for the dev capture; confirm a production deploy shows the real engine + prefix.
  - Chris: Yeah I don't care about this
  - **DONE (post-AO.S, code on `main`).** Operator-noted as low-priority. Closed via code-inspection: `common/sheets/app_info.py:311-312` reads `cfg.dialect.value` + `cfg.deployment_name` (no hardcode). Added lock test `test_app_info_deploy_stamp_reads_dialect_and_prefix_from_cfg` parametrized over the 4 shipped apps × 2 cfg shapes (PG/recon-prod, Oracle/recon-staging) to gate against a future hardcode regression. No AWS deploy needed.
- [x] AO.C2 - (feedback #9) Empty-default AML sliders (Fanout, Anomalies) — confirm render-on-default vs broken on AWS (likely the slider-default class; pairs with AO.9).
  - **DEFERRED to the next cold-read AWS pass.** AO.R.4 fixed the App2-side slider-default class (live confirmed: σ-anomaly KPI default→129, σ=2→129, σ=3→67). For QS-on-AWS, the slider-default class is a QS engine feature; if the empty-default rendering is broken, it'd surface as a QS-side parameter-control quirk (per `project_qs_url_parameter_no_control_sync` memory). Punting visual confirmation to the next deploy + cold-read team's pass; if they flag it on AWS, queue a follow-on. Cold-read pre-flight: local-only per operator decision 2026-05-24, so QS-on-AWS is exercised on the operator's deploy schedule.
...
- [x] BC.7 - **Production fix: `recon-gen` populates `<prefix>_config`.** Surfaced during BC.6 — the L1 matviews (limit_breach, stuck_pending, stuck_unbundled) all JOIN to `<prefix>_config.l2_yaml` via `JSON_TABLE`; the production CLI path never populated it. Agent (worktree `agent-a912238db8512ec7f`) delivered the surface; integrated 2026-05-24 with a wiring correction.
  - [x] BC.7.1 - Option (b): new `emit_config_populate_sql(*, prefix, cfg_json, l2_json, as_of, dialect) -> str` in `common/l2/config_table.py` (static SQL via `execute_script`; sqlite3-typed `replace_config` kept untouched for the ~7 test_spine_*.py fixtures).
  - [x] BC.7.2 - Wired into **`schema apply`** (NOT `data apply` as the agent's pre-clarification brief said — user's three-event lifecycle locks the populate to the deploy event). New `build_config_populate_sql(cfg, instance, *, anchor)` helper in `cli/_helpers.py`; `schema_apply` concatenates schema DDL + populate SQL into one stream.
  - [x] BC.7.3 - `tests/audit/test_data_apply_populates_config.py::test_data_apply_populates_config_and_limit_breach` — drives the real Click CLI against a postgres:17-alpine testcontainer; asserts config-row count == 1 AND `limit_breach` is non-empty.
  - [x] BC.7.4 - `apply_db_seed` work-around fully retired: routes through the same `emit_config_populate_sql` + `serialize_l2` path the production code uses; no inline DELETE+INSERT, no per-rail Duration walk.
- [x] BC.8 - **Production fix: `serialize_l2` emits `*_seconds` derived fields on rails.** Agent (worktree `agent-a37d8e6d92a060392`) delivered the surface; integrated 2026-05-24.
  - [x] BC.8.1 - Option (a): extended `serialize_l2` (one serializer, no drift). Round-trip tests verified field-equal (loader has no unknown-key whitelist; `_seconds` ignored on the round-trip).
  - [x] BC.8.2 - Walked every Rail's Duration field; `_seconds` companion added when the parent field emits. Scanned the matview SQL: only two Duration→numeric fields exist (the two age caps); `cap` on limit_schedules is already Decimal/Money. No surprise extra surface.
  - [x] BC.8.3 - 5 new tests in `tests/unit/test_serialize_l2_emits_seconds_fields.py`: emission walk + symmetric absence + round-trip + 2 end-to-end matview drives (stuck_pending + stuck_unbundled).
  - [x] BC.8.4 - Duration→seconds walk hack in `apply_db_seed` retired (the BC.7 integration replaces it with `serialize_l2` → yaml.safe_load → json.dumps).
- [x] BC.9 - Narrow temporal-tuple sweep. `StudioState.window_start: date | None + window_end: date | None` collapsed to `window: DateInterval | None` (the "both set together or both None" invariant is now enforced by the type). `TestGeneratorCache._window_start / _window_end` collapsed to `_window: DateInterval`; `get_window() -> DateInterval` (was `tuple[date, date]`); `update_window` returns `DateInterval`. yaml sidefile shape preserved (`trainer_window: {start, end}` — operator-readable); parser/serializer converts at the boundary. 6 callers in `_studio_routes.py` updated (5 `get_window` unpacks + 1 nested + 1 update_window). 5 test callers updated (`test_tg_cache.py`, `test_studio_state.py`, `test_studio_data_route.py`). Skipped: `common/tree/date_view.py::required_coverage` + `RollingDate` defaults — BD scope (tangled with the AO.11 `(as_of, window)` frame). 2831 unit+audit green.
- [x] BC.11 - **CI fix: integration-pg sq_pg_lo `StringDataRightTruncation: value too long for type character varying(100)`.** Reproduced locally against postgres:17-alpine + sasquatch_pr — the overflow value was `tx-chainfill-xfer-limit-breach-CustomerInboundACH-Inbound-cust-0001-snb-CustomerInboundACHReturnNSF-0` (exactly 101 chars), an id synthesized by the chain-completion plant adapter when L2 rail names are verbose. spec_example's shorter names fit; sasquatch's didn't.
  - [x] BC.11.1 - Bisected to stmt #131244 via line-by-line replay; overflowing value identified (`id` column on `<prefix>_transactions`).
  - [x] BC.11.2 - Widened `id` / `transfer_id` / `transfer_parent_id` / `bundle_id` from `vc100` to `vc255` in `common/l2/schema.py::_BASE_TABLES_TEMPLATE`. These are generator-synthesized via the chain-completion / xor-completion adapters (`f"tx-chainfill-{parent_transfer_id}-..."`). The other vc100 columns (account_id / account_role / rail_name / template_name) stay narrow — they're L2-declared identifiers, not synthesized chains. Schema-test asserts updated to match (2 sites).
  - [x] BC.11.3 - `tests/data/test_seed_id_column_widths.py::test_seed_emit_respects_column_widths` — emits the seed SQL for spec_example AND sasquatch_pr, walks every INSERT, asserts no string VALUE exceeds the declared column width. Catches the next overflow (whether sasquatch grows again, fuzz produces a longer name, or a future generator synthesizes wider IDs).
- [x] BC.12 - **CI fix: integration-oracle ORA-32368 cleared.** ORA-32368 no longer reproduces against `gvenzl/oracle-free:23-faststart`. Worktree agent delivered the surface; integrated 2026-05-24 with a small refactor (schema.py routes through the existing `build_config_populate_sql` helper rather than the agent's inlined version). Architecture per BC.12.2's lock: EAV storage (`<prefix>_config_kv`) + typed projection views (plain self-join, not recursive CTE — L2 paths are depth-2) + matview JOINs against the views. Three production-side bugs surfaced behind the original ORA-32368 wall — see BC.12.4-6 + BC.14.
  - [x] BC.12.1 - Stmt #91 identified as `CREATE MATERIALIZED VIEW <prefix>_stuck_pending` (NOT limit_breach as I'd guessed). Reproduced against `gvenzl/oracle-free:23-faststart` at port 60643.
  - [x] BC.12.2 - **Constraint locked: Oracle 19c is the floor** ([[project_oracle_19c_compat]]) — captured in the spike doc + the lived-experience writeup at `docs/reference/oracle-19c-constraints.md` + the auto-memory. The Oracle 21c+ native `JSON` column type is NOT available; the `<prefix>_config.l2_yaml` column has to stay `CLOB` (Oracle) / `TEXT` (PG). Oracle 19c's `JSON_TABLE` works as a SELECT-time expression but Oracle's matview engine rejects matviews whose source is a JSON_TABLE on a CLOB literal/subquery — ORA-32368.
    ...
    **Three-event production lifecycle (locked 2026-05-24):**
    1. **Once** — `schema apply --execute` creates the 3 base tables: `<prefix>_config_kv` + `<prefix>_transactions` + `<prefix>_daily_balances`. DDL bootstrap.
    2. **Every deploy** (L2 changes) — `schema apply --execute` re-creates all views + matviews AND re-populates `<prefix>_config_kv` from `--l2`. Heavy but idempotent. **kv populate lives in `schema apply`, not `data apply` or `data refresh`** — it's a deploy-event, not an ETL-event.
    3. **Daily** (post-ETL) — `data refresh --execute` REFRESHes matviews. Cheap, rerunnable, no kv touch.

    (Subtle: `schema apply` currently drops the base tables too — that loses transaction data in production. Separate concern from BC; worth noting for a future "split DDL bootstrap from view/matview rebuild" pass.)

    **BC.7 coupling**: BC.7 landed the JSON-as-CLOB populate path (`<prefix>_config(as_of, cfg_yaml, l2_yaml)` row). BC.12 replaces this with the EAV populate (`<prefix>_config_kv` rows). BC.7's `emit_config_populate_sql` + `build_config_populate_sql` helpers get swapped underneath; the `schema apply` call site already in place doesn't need to change.
  ...
  - [x] BC.12.3 - **Spike landed at `docs/audits/bc_12_config_kv_spike.md`.** Decisions: `value` = `CLOB` (Oracle) / `TEXT` (PG/SQLite) — sasquatch's per-field max is ~989 chars but the deferred `l2_yaml_raw` provenance row is ~37KB; CLOB it is. `node_id` = Python-side monotonic counter (no DB sequence). **Typed views use plain self-join, NOT recursive CTE** — L2 matview-consumed paths are depth-2 (rails[i].x, limit_schedules[i].x), so a 2-table self-join suffices. New `lob_substr` helper in `common/sql/dialect.py` for `MAX(CLOB)` coercion (Oracle ORA-22849). New `kv_as_of_as_timestamp_sql` for `TO_TIMESTAMP(DBMS_LOB.SUBSTR(...))` (Oracle ORA-00932 — `CAST(CLOB AS TIMESTAMP)` rejected).
  - [x] BC.12.4 - **`emit_config_kv_ddl` replaces `emit_config_table_ddl`.** Done in `common/l2/config_table.py`. `<prefix>_config_kv` shape: `node_id BIGINT PK`, `parent_id BIGINT NULL` (self-FK), `key VARCHAR(255)`, `value` (CLOB Oracle / TEXT PG+SQLite per BC.12.3). Index on `(parent_id, key)`. Old `<prefix>_config` DDL dropped; existing deploys had it empty (BC.6 surfaced the gap), so no data-loss migration.
  - [x] BC.12.5 - **`emit_config_populate_sql` walks the L2 + INSERTs kv rows.** Body rewritten. Walker emits N kv rows per JSON node. `as_of` at top-level (`parent_id=NULL`), rail/limit_schedule leaves at their natural depth. `cfg_json` arg vestigial (matviews don't read cfg today). **Deferred**: `l2_yaml_raw` + `cfg_yaml_raw` opaque-provenance rows. The Oracle chunked `TO_CLOB(c1) || TO_CLOB(c2) || ...` shape required for ORA-01704 (~4000-char string-literal cap) collides with `batch_oracle_inserts`'s quote-aware coalescer. Operators retain access to the original yaml via their `--l2 <path>` source file; the typed projection views give matviews the data they actually consume. Backlogged below.
  - [x] BC.12.6 - **Typed projection views shipped** in `emit_schema`:
    - `<prefix>_v_config_limit_schedules(parent_role TEXT, rail TEXT, direction TEXT, cap NUMERIC)` — for `limit_breach`.
    - `<prefix>_v_config_rails(name TEXT, max_pending_age_seconds BIGINT, max_unbundled_age_seconds BIGINT)` — for `stuck_pending` / `stuck_unbundled`.
    **Plain self-join, not recursive CTE** (spike finding — L2 paths are depth-2; recursive overkill).
  - [x] BC.12.7 - **Matview DDLs JOIN typed views.** `limit_breach` / `stuck_pending` / `stuck_unbundled` rewritten in `common/l2/schema.py`. Side-fix: 4 sites of `todays_exceptions` matview's `CAST(NULL AS BIGINT)` (Oracle rejects — BIGINT not a valid Oracle type) routed through new `{null_bigint}` placeholder. Stmt #91 (ORA-32368) cleared; exposed stmt #125 (BIGINT) — fixed in the same scope. Other JSON_TABLE callers (`cli/audit/__init__.py`, `apps/`) left as-is since they're plain SELECTs (Oracle accepts JSON_TABLE in non-matview queries).
  - [x] BC.12.8 - **Verified.** `pytest tests/unit tests/audit tests/data tests/l2 tests/json tests/schema` → 3731 passed (2837 + 953 across the layers locally post-integration). `./run_tests.sh up_to=db --variants=sp_pg_lo,sp_sl_lo` both green (49/49 e2e each). `./run_tests.sh up_to=db --variants=sp_or_lo` → **schema apply now succeeds** (ORA-32368 gone). Data apply on Oracle hits a SEPARATE pre-existing bug (BC.14 below). Original ORA-32368 repro no longer fires.
  - [x] BC.12.9 - **`apply_db_seed` works against the new kv shape** by virtue of `emit_config_populate_sql`'s body rewrite — no fixture-side code change needed. `test_aw1_config_table.py` updated for the new assertion shape (335-line rewrite).
  - [x] BC.12.10 - **Docs landed.** `docs/audits/bc_12_config_kv_spike.md` (spike) + `docs/reference/oracle-19c-constraints.md` (lived experience). `project_oracle_19c_compat` memory updated post-integration (agent's sandbox blocked the edit; I did it after).
- [x] BC.13 - Commit + tag. `__version__` bumped to 11.19.0; RELEASE_NOTES.md drafted with the full BC.0-BC.14 story + migration/breaking-changes section. Tagged + pushed 2026-05-24.
- [x] BC.14 - **Oracle data-apply ORA-01843 fixed** — spine's dry-run renderer now detects `YYYY-MM-DD HH:MM:SS` strings and wraps with ANSI `TIMESTAMP '...'` for Oracle (PG/SQLite emit bare quoted strings as before). The renderer's docstring even predicted this gap (lines 25-31, AY.4.b shipped acknowledging Oracle TIMESTAMP wrapping was unhandled). Surfaced as ORA-01843 at stmt #2368 of data apply post-BC.12.
  - [x] BC.14.1 - Reproduced against `quicksight-test-oracle-sp_or_lo` (port 60643). Failing literal: bare `'2026-05-24 00:00:00'` in `INSERT INTO <prefix>_daily_balances`. Hypothesis confirmed — Oracle's default NLS_DATE_FORMAT (`DD-MON-RR`) rejects the spine's canonical timestamp shape.
  - [x] BC.14.2 - Fixed in `common/spine/dry_run_renderer.py::_render_literal`. Shape-detected via regex `^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}(?:\.\d+)?$`; wrap with `TIMESTAMP '...'` only when `dialect is Oracle`. Chose ANSI wrap over `TO_TIMESTAMP(..., 'YYYY-MM-DD HH24:MI:SS')` because the ANSI form is shorter, equally unambiguous, and matches how Oracle's own docs recommend literal-form timestamps.
  - [x] BC.14.3 - Verified end-to-end: schema apply → data apply → data refresh all green against the Oracle container with spec_example. 22 unit tests in `test_spine_ay4b_dry_run_renderer.py` (5 new BC.14 tests: Oracle-wrap-timestamp, Oracle-don't-wrap-non-timestamp, Oracle-wrap-microseconds, PG-don't-wrap, SQLite-don't-wrap).
...
- [ ] BD.3 - Plant emit threads `AsOfFrame`. `scenario_to_generators(frame=...)` — legacy `anchor` / `plant_window` / `as_of` kwargs dropped (no-compat-shim). All 4 production callers migrated: `cli/data.py::semantic-lock`, `common/l2/seed.py::emit_seed` + `::emit_full_seed`, `tests/e2e/_seed_helpers.py::apply_db_seed`. Semantic locks regenerated to capture BC.4-corrected per-plant spreading (pre-BD.3 lock encoded the latent "all plants stack on anchor" bug per the BC.4 docstring).
