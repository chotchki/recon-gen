...
- [ ] BC.12 - **CI fix: integration-oracle `ORA-32368: cannot create JSON materialized view without relational table` at schema-apply stmt #91.** Affects both spec_example AND sasquatch on Oracle; means EVERY Oracle CI cell has been red since whenever this regressed. **Coupled to BC.7 — see BC.12.2's shape note.** Diagnostic-first:
  ...
  - [ ] BC.12.3 - **Spike + lock the kv schema.** Spike the EAV table DDL across PG / Oracle / SQLite. Pin the column types: `node_id BIGINT PRIMARY KEY` (sequence/identity on PG+Oracle, INTEGER PK on SQLite), `parent_id BIGINT NULL` (self-FK), `key VARCHAR(255) NOT NULL`, `value` type — open question: `VARCHAR(4000)` (matches the existing metadata cap) vs `CLOB/TEXT` (handles l2_yaml_raw provenance row that could be larger). Spike confirms which works as a matview source on Oracle 19c. Doc the answer + decision in `docs/audits/bc_12_config_kv_spike.md`.
  - [ ] BC.12.4 - **Replace `emit_config_table_ddl` with `emit_config_kv_ddl`.** `common/l2/config_table.py`: drop the 3-column `<prefix>_config` DDL; emit `<prefix>_config_kv` per BC.12.3's pinned schema. Index on `(parent_id, key)` for the recursive-CTE join. Keep the `config_table_name(prefix)` helper but point it at the new table. `emit_config_table_drop` updates similarly. Migration: existing deploys (if any) lose their `<prefix>_config` row — but since BC.7 only just shipped the populate, NO existing customer deploy has a populated config row to lose; the "migration" is just "schema apply --execute against the v11.19.0 schema."
  - [ ] BC.12.5 - **`emit_config_populate_sql` writes to kv.** Replace BC.7's `<prefix>_config (as_of, cfg_yaml, l2_yaml) VALUES (...)` with the walk-the-L2 + INSERT-N-rows path. Walk: every JSON node → one kv row with `(node_id, parent_id, key, value)`. Top-level scalars (`as_of`, `description`) at `parent_id=NULL`; nested objects/arrays at their natural depth. Pre-compute node_ids in Python so the SQL is one giant `INSERT ... VALUES (...), (...), ...` batch (or per-row INSERT — measure). The `emit_config_populate_sql` signature stays the same; its body changes from one INSERT to N INSERTs. (The `cfg_json` arg becomes vestigial — matviews don't read cfg today; pass-through but no longer goes to a `cfg_yaml` column. Document.)
  - [ ] BC.12.6 - **Typed projection views (pay-as-you-go).** Emit two views in `emit_schema`:
    ...
    Each view body: `WITH RECURSIVE walk_kv AS (...) SELECT pivot(...)`. Use the existing `recursive_with_cte` dialect helper (or add one in `common/sql/dialect.py` if absent — Oracle's `WITH` is recursive implicitly; PG/SQLite need `RECURSIVE`). Add only these two views; future L2-consuming matviews add their own typed view alongside the matview that needs it.
  - [ ] BC.12.7 - **Rewrite the matview DDLs to JOIN typed views.** `common/l2/schema.py`: change `limit_breach` / `stuck_pending` / `stuck_unbundled` matview SQL to JOIN against `<prefix>_v_config_*` instead of JSON_TABLE-ing `<prefix>_config.l2_yaml`. Other JSON_TABLE callers (audit-side queries in `cli/audit/__init__.py`, dashboard datasets in `apps/`) — survey + either migrate them to the kv (or its views), OR leave them on whatever in-DB shape they currently use if it's not Oracle-matview-blocked.
  - [ ] BC.12.8 - **Verify all three dialects.** `./run_tests.sh up_to=db --variants=sp_or_lo,sp_pg_lo,sp_sl_lo` all green. Then reproduce the original ORA-32368 scenario by hand against the Oracle container (sasquatch L2; check stmt #91 no longer fails). Bonus verify: BC.6's chronic e2e gate (postgres-) still passes with the rewritten matviews.
  - [ ] BC.12.9 - **Update `apply_db_seed` test fixture for the new shape.** The fixture currently calls `emit_config_populate_sql` (which BC.12.5 changed to write kv). Should "just work" by virtue of the populate helper's body change; verify + add any extra fixture-side init that the new shape requires (e.g., the matview tests that previously checked `<prefix>_config.l2_yaml` contents need to check kv contents instead — likely few/zero callers).
  - [ ] BC.12.10 - **Memory + docs.** Update [[project_oracle_19c_compat]] with the lived experience (already done). New `docs/reference/oracle-19c-constraints.md` or similar — "we built the EAV layer to dodge ORA-32368; here's the trigger condition for deleting it." Surfaced in [[feedback_quirks_log_ever_growing]] cadence as part of the same PR.
