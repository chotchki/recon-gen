...
Both are now unrepresentable. There is ONE `DbObject` graph; the five emit orders are derived from it by a topological walk, so a divergence can't be authored — an inverted edge raises at import. The CREATE bodies emit through that same graph walk, so create is the exact reverse of drop by construction, not by a human keeping two lists in sync. And every column reference resolves against the object's declared column set: a `contract_from` builder derives each dataset contract's columns from the matview it actually reads (source DERIVED from the SQL, never guessed), so a matview column rename is a `KeyError` the moment the app module imports — not a blank visual at deploy. A SQLGlot lineage lint (test-tier only, never in the wheel) closes the other direction: every matview body and every dataset SQL is parsed against the object catalog, and an unresolved WHERE / JOIN / GROUP-BY reference — the surface the old text-scan projection guard never saw — fails at the unit tier.

None of it changes a byte the product emits. The whole refactor is gated by a byte-identity check across all three dialects plus the DuckDB semantic lock: the emitted schema, the refresh SQL and the drop SQL are proven identical before and after, with exactly ONE deliberate, documented exception — an independent-matview CREATE pair canonicalizing to the graph's order, verified a pure reorder. The typed model is testing-before-the-test: the footguns close at the wiring site, in the type system, instead of in a suite that walks the generated output after the fact.

This release also opens the EA phase — CI moving back off the single self-hosted box toward GitHub-hosted cloud runners, which the QuickSight/AWS removal finally unlocked. The shipped-in-this-cut groundwork is a self-signed-TLS path for App2's own auth on a runner with no DNS control (env-gated, a hard no-op everywhere the real ACME cert is used) and the timings-as-signal change: the unit tier's wall-clock budgets, which were calibrated to one fast machine and red-herring on any slower one, now RECORD their measurement and flag drift instead of hard-failing on an absolute. The runner flip itself is still ahead; the reasons the box existed are gone.

### Added

- The typed `DbObject` dependency graph (`common/db_objects.py`): one frozen node per schema object owning `depends_on` (object refs — a cycle is unconstructable, a missing dep is a NameError at authoring) + emitted `columns`. The refresh / DuckDB-analyze / L1-drop / inv-drop / overlay-restore / create orders all derive from this one graph; the four former hand-copies are retired.
- `DbObject.emit_create` + a load-time template carve: the two monolithic matview `.format()` templates split into per-object CREATE bodies so `emit_schema` walks the graph create-order, making create the exact reverse of drop.
- `common/contracts.py` — `contract_from(source, [keep/dollars/added])`: a dataset contract DERIVES its sourced columns from the emitting matview's declared columns, so a matview column rename raises `KeyError` at import (proven end-to-end). Migrated the 20 clean single-source contracts across the four apps; each source derived from the dataset's real `FROM`/`JOIN`, byte-verified against the prior literal.
- The SQLGlot column-lineage lint (`tests/unit/test_dq5_sqlglot_lineage.py`, `sqlglot` in the dev extra, `importorskip`-guarded): matview bodies + dataset SQL parsed against the object catalog; unresolved refs fail, and every matview projection + every dataset projection is asserted to equal its declared columns (replacing and strengthening the old one-directional regex projection guard, now retired).
- A `LinkedValues` picker-column gate (`test_dq44_linked_values_resolve.py`): every dropdown's `options_column` resolves against its source dataset's contract — the one column-rename surface the lineage lint couldn't reach.
- EA groundwork — a self-signed TLS path (`RECON_GEN_TLS_SELF_SIGNED`, `RECON_GEN_BROWSER_INSECURE_TLS`; env-gated) minting a cert for the locked CI SANs with no Cloudflare/ACME, and `tests/_timing_signal.py` demoting the box-calibrated unit-tier wall-clock budgets to captured-and-flagged signals.

### Changed

- The emitted schema / refresh / drop SQL is byte-identical to v16.2.0 across PostgreSQL / Oracle / DuckDB, with one documented exception: `expected_eod_balance_breach` and `balance_cadence_gap` (mutually independent matviews) now CREATE in graph order, a verified pure reorder — the DuckDB semantic lock is unchanged.
- `EXC_DEAD_LIMIT_SCHEDULES.cap` renders as currency (`$`) — it's the L2-declared limit cap, a money value.
...
- **DuckDB local-iteration backend** with **54.9× faster bundled
  refresh at ~1M rows** vs the SQLite baseline (4.6s vs 252.9s) and
...
- **`isolated_cfg`** fixture + `IsolationScope.AGREEMENT_AUDIT` /
  ...
  declares it writes**, not on cell-shared prefixes. Closes #199
...
- **Pre-init pass** boots the freshly-built image once, waits for
  ...
  then `docker commit`s the writable layer back onto the same tag.
...
surfaced. AY's re-locks (twice during the phase) exposed the
byte-locked seed files (`tests/data/_locked_seeds/*.sql`, ~28 MB
across 3 dialects) as friction: they encoded SQL formatting
decisions that aren't load-bearing, and every documented drift
required a 28 MB diff to review. AZ replaces them with per-(L2
instance, dialect) JSON semantic locks that gate on the
**violation set** the spine detectors produce post-emit, not on
SQL bytes.

**New gate file format** (~18-19 KB each):

```
tests/data/_semantic_locks/<instance>.<dialect>.json
{
  "_comment": "Auto-generated by `recon-gen data semantic-lock`.",
  ...
  "violations": {
    "drift": [
      {
        "kind": "rule_violation",
        "identity": {"account_id": "...", "business_day": "2030-01-02", "drift": 5.0}
      }
    ],
    "fan_in_chain_healthy": [
      {"kind": "coverage", "identity": {...}}
    ],
    "supersession": [
      {"kind": "audit_fixture", "identity": {...}}
    ]
  }
}
```

**Key design decisions** (AZ.0 spike, full doc at
`docs/audits/az_0_semantic_lock_schema.md`):

- Per-violation `{kind, identity}` shape. `kind` maps directly to
  AY.2.a Violation subtypes (`rule_violation` / `coverage` /
  `audit_fixture`).
- JSON-string equality is the gate contract. No `Violation` object
  reconstruction; the canonical serialization IS the truth.
- Mandatory sort order (invariant alpha, violations by `(kind,
  sorted-identity-repr)`, identity keys alpha) — re-emit is byte-
  stable.
- `_comment` field at top-level (JSON has no comments) for
  reviewer context.
- `scenario_fingerprint` for cheap pre-check on instance / dialect
  / anchor / schema_version before the expensive emit.
- Empty invariants kept as empty arrays for diff visibility ("X
  used to fire, doesn't now").

**New surface**:

- `src/recon_gen/common/spine/semantic_lock_json.py::lock_to_json`
  — renders `dict[str, frozenset[Violation]]` → canonical JSON.
- `recon-gen data semantic-lock --l2 <yaml> [--check]` CLI —
  mirrors the retired `data lock`. Default writes; `--check`
  exits non-zero on drift with unified diff to stderr.
- `tests/data/test_semantic_locks.py` — auto-discovers every
  `_semantic_locks/*.sqlite.json` + parametrizes both the byte-
  equality check + a fingerprint pre-check over the lock corpus.

**Retired** (AZ.5):

- `tests/data/_locked_seeds/{spec_example.sqlite,postgres,oracle}.sql`
  (3 × ~9.4 MB)
- `tests/data/test_locked_seeds.py` (the byte-equality gate)
- `recon-gen data lock` CLI command + `_LOCKED_SEEDS_DIR` constant

Net disk: ~28 MB of seed SQL deleted; ~38 KB of semantic-lock
JSON added (2 baseline files). 99.9% reduction.

**Scope**: SQLite-only initially. The matview SQL differs per
dialect so PG / Oracle semantic locks need real deployed DBs
(the deploy_pipeline path); AZ.1.b extension lands those if a
post-AZ regression surfaces. The byte-lock gate had PG / Oracle
coverage; the semantic-lock gate covers the dialect-agnostic
violation contract, which is what matters in practice (matview SQL
differences that change violation sets would surface as test
failures elsewhere — the 4-way agreement gate + e2e dashboard
checks).

**Validation table** (from AZ.0) — semantic lock catches every
real change AY.5's byte locks caught + drops every byte-only
false positive:

| Drift source              | Byte lock | Semantic lock |
|---------------------------|-----------|---------------|
| per-row INSERT shift      | false-pos | NO ✓          |
| header text change        | false-pos | NO ✓          |
| metadata payload addition | false-pos | NO ✓          |
| real violation change     | YES       | YES ✓         |
| new invariant added       | YES (huge)| YES (1-key)   |
| violation drops out       | YES       | YES ✓         |

**Tests:** 2804 unit + data pass (was 2812 with both gates
running during AZ.3's dual-validation window; -8 byte-lock tests
gone post-retirement). 0 failures. pyright strict clean.

**Next:** Phase AT.5 4-way agreement gate continues to anchor
cross-stack semantic equivalence (QS / App2 / PDF / direct-DB);
unchanged by AZ.
...
**New L1 invariant — xor_group_violation.** `<prefix>_xor_group_violation` matview surfaces any Transfer where a declared group has 0 firings (missed closure) or ≥2 firings (overlap). CTE inlines `(template_name, xor_group_index, member_rail_name)` rows from the L2 yaml, LEFT JOINs them against `<prefix>_current_transactions` per (Transfer-of-template, group-of-template, member-rail), and gates on `HAVING COUNT(tx.transfer_id) <> 1`. When no template declares groups, the body short-circuits with `WHERE 1=0` (parses cleanly across PG / Oracle / SQLite, zero rows). Projection: `transfer_id` / `template_name` / `xor_group_index` (0-based) / `firing_count` (0 = missed; ≥2 = overlap) / `fired_rails` (dialect-specific concat of variants that DID fire) / `business_day`. 3 hot-path indexes (transfer / template / day). Wired into `refresh_matviews_sql` (PG/Oracle + SQLite) + `_L1_INVARIANT_DROP_NAMES`. Matview count bumped 16 → 17 across both refresh assertions.

**Validator C1 rewrite + C1a-d.** C1 ("at most one Variable-direction leg per template") rewrites to "at most one *non-grouped* Variable leg" — members of `leg_rail_xor_groups` are exempt from the cap (the per-firing pick keeps closure determined). New structural rules: C1a (member ⊆ leg_rails) / C1b (Variable-direction SingleLegRail only) / C1c (no rail in two groups within one template) / C1d (≥2 members per group). Runtime "exactly one fires per Transfer" check stays out of the load-time validator — it lives in the matview.

**Seed — per-firing deterministic variant pick.** New `_xor_suppressed_members(template, *, firing_id)` helper computes the per-firing pick via `zlib.crc32(f"{template_name}|{gi}|{firing_id}".encode())`. Firing key = `parent_transfer_id` (not a global counter) — stable per chain invocation regardless of seed / scope, so re-running with `scope: only_template` (or any subset) doesn't shift which variant fires for any given firing. Picker is RNG-independent. `_emit_chain_child_template_legs` consults the suppressed set before each `leg_rails` iteration and skips suppressed members; templates with empty `leg_rail_xor_groups` see no behavioral change.

**Plants + scenario.** Two new dataclasses in `seed.py`:
- `XorVariantMissedFiringPlant` — emit ONE witness leg row tagged with the template_name but no member of the targeted group; matview row carries `firing_count=0`, `fired_rails=''`.
- `XorVariantOverlapPlant` — emit TWO leg rows sharing one `transfer_id` + `template_name`, both rail_names being members of the target group (postings 1 minute apart for visual separability); matview row carries `firing_count=2`, `fired_rails='<a>,<b>'`.

`auto_scenario.py` derives `_pick_xor_missed_firing_inputs` (first XOR-grouped template with a non-XOR leg_rail as witness) and `_pick_xor_overlap_inputs` (first XOR group with ≥2 members — validator C1d guarantees every group qualifies). 6 pass-through call sites per plant: default_scenario_for / densify_scenario / boost_inv_fanout_plants / add_broken_rail_plants / filter_scenario_plants. `tests/l2/spec_example.yaml` declares `SettlementTimingCycle` with `[SettlementAuto, SettlementStandard]` XOR group + `SettlementSlow` as the lone non-grouped Variable witness. `tests/l2/sasquatch_pr.yaml` extends `MerchantSettlementCycle` with TWO 3-member XOR groups: settlement-timing trio + fraud-review trio (6 variants, multi-group axis exercising a different shape than spec_example).

**Dashboard wiring.** `<prefix>_todays_exceptions` matview gains a UNION ALL branch projecting `check_type='xor_group_violation'`, `template_name AS rail_name`, `firing_count AS magnitude`. Today's Exceptions surfaces violations alongside chain_parent_disagreement automatically — no dedicated sheet (same UNION-only pattern). L1 Pending Aging + Unbundled Aging bar charts gain `colors=[rail_name]` + `bars_arrangement="STACKED"`: each age bucket bar is now stacked by rail_name, so per-variant rollups for XOR-grouped multi-Variable templates show up as distinct color bands ("the slow variant is dragging" reads off the chart).

**Studio editor (leg 2 of the contract).** New `multi_select_groups` FieldKind + `FieldSpec.edit_only` flag. TransferTemplate card gains a `leg_rail_xor_groups` field: one `<fieldset class="xor-group">` per existing group with a `multi-select-group` checkbox grid sourcing the template's own `leg_rails`, plus a trailing always-empty `<fieldset class="xor-group new">` slot for "Add new XOR group" — no JS required for add/remove (uncheck-all-boxes-to-delete UX; hidden `__num_groups` tells the server how many slots were rendered). `edit_only=True` filters the field off the create page (operator authors `leg_rails` first, then edits to add the group layer). Read card displays nested groups as a bullet list ("group 1: A, B") rather than the flat-tuple repr.

**Topology + diagram.** Typed projection (`topology_graph_for`) tags `template_member` edges with `xor_group_index` metadata when the rail is in a `leg_rail_xor_groups` entry; non-grouped rails get no key (absence IS the not-grouped signal). Graphviz per-rail renderer (`build_topology_graph_per_rail`) wraps XOR-grouped rails in a nested sub-cluster inside the template cluster, labeled "XOR group N (exactly 1 fires)" with a distinct blue-grey hue + dashed membership edges. The mutual-exclusion contract reads off the diagram directly.

**Fuzzer.** New `_maybe_inject_xor_template` post-processor injects ONE synthetic `TransferTemplate_FuzzXor` (3 Variable-direction SingleLegRails `FuzzXorVariant{Auto,Standard,Slow}` all sharing one leg_role + origin + transfer_key, with `leg_rail_xor_groups=[[v0..v_n]]` where n is randomized to 2 or 3) with ~50% probability per seed. ~half of `META_GUARD_SEEDS` exercise the AB.3 paths. Meta-guard test gains `xor_grouped_template` saw entry.

**Docs.** `concepts/l2/transfer-template.md` gains a new field description for `leg_rail_xor_groups` + a "Multi-mode templates (AB.3): one closure, several variants" section. New `walkthroughs/customization/how-do-i-add-multi-mode-settlement.md` (3-step recipe + verify + footgun list). `L1_Invariants.md` gains section 9 mirroring section 8's shape. `SPEC.md` `LegDirection = Variable` distinguishes non-grouped (C1) vs grouped (exempt) Variables + documents C1a-d. `Schema_v6.md` cross-links the `template_name` column to the new matview. `INVARIANT_KIND_TO_SHEET` + `_L1_KIND_TO_SHEET_ID` + `_DISPLAY_ORDER` all carry the new kind (Today's Exceptions deep-link, displayed right after chain_parent_disagreement).

**Tests.** 8 new validator tests (C1 positive + negative; C1a / C1b / C1c / C1d isolated). 3 new schema-shape tests (empty-branch placeholder + populated CTE shape + Today's Exceptions UNION). 8 picker-shape tests (deterministic per firing_id, distribution, RNG-independence, 2-member symmetry, template_name in key). 7 Studio-editor tests (edit form render + create form omission + PUT round-trip + validator-rejection inline error + read card bullets + 2 direct coerce-form unit tests). 2 topology tests (typed projection metadata + graphviz sub-cluster source). 1 dashboard wire test (Pending + Unbundled Aging stacked bars + rail_name colors). Bundled-doc kinds pin updated. Verified at 2915 passed across unit + data + schema + audit + json + cli layers; live PG + AWS QS verify happens when CI runs the e2e + agreement leg on this tag.

**Re-locked seeds:** `tests/data/_locked_seeds/spec_example.{postgres,oracle,sqlite}.sql` regenerated when AB.3.5.spec activated the XOR group; AB.3.7-AB.3.10 (Studio / topology / dashboard / docs) don't touch seed emission, so the lock stays byte-equal through close-out.

## v11.2.0 — AB.2 template-as-chain-child + chain_parent_disagreement L1 invariant

Feature release. `Chain.children` may now resolve to a `TransferTemplate` (previously: silent-skip downstream). When the chain singleton-child is a template, every leg_rail firing of that template aggregates into one shared child `Transfer` via first-firing-wins per gap doc §3 — the first leg's `parent_transfer_id` is the chain Parent, and subsequent legs reuse the same value via lookup-or-create on `(template_name, parent_transfer_id, business_day)`. **Fully additive** — pre-AB.2 L2 yamls with rail-only chain children behave byte-equivalent through load/seed/dashboard.

**New L1 invariant — chain_parent_disagreement.** `<prefix>_chain_parent_disagreement` matview surfaces any child Transfer where leg_rail firings disagree on which parent firing they descend from (ETL bug: stale parent reference, cross-cycle contamination, race condition). `GROUP BY (transfer_id, template_name)` over `<prefix>_current_transactions` with `HAVING COUNT(DISTINCT transfer_parent_id) > 1`. Filters: `transfer_parent_id IS NOT NULL`, `template_name IS NOT NULL` (rail-as-child chains don't false-positive), `status <> 'Failed'`. Projection: `transfer_id` / `child_template_name` / `business_day` (MIN posting day) / `distinct_parent_count` / `parent_transfer_id_min` / `parent_transfer_id_max` (sample conflicting values so the analyst can drill without re-running the GROUP BY). 3 hot-path indexes (transfer / template / day). Wired into `refresh_matviews_sql` (PG/Oracle + SQLite paths) + `_L1_INVARIANT_DROP_NAMES`. Matview count bumped 15 → 16 across both refresh assertions.

**Validator — auto-derived parent_transfer_id.** When `chain.children` contains a singleton TransferTemplate, every leg_rail of that template implicitly accepts `parent_transfer_id` as a posted metadata key (auto-derived from the chain relationship, AB.2.0 design lock — no operator yaml declaration needed). The existing `derived.posted_requirements_for` already handled this case via `chain_targets` including both rail name AND containing template names; AB.2.2 added an explicit multi-leg-rail test asserting all legs inherit the requirement.

**Seed — two-template chain firings.** New `_emit_chain_child_template_legs` replaces the pre-existing silent-skip path at `seed.py:2442-2446`. Generates ONE shared `transfer_id` per chain invocation, iterates the child template's leg_rails, calls the existing `_emit_chain_child_leg` once per leg with `shared_transfer_id=` and `template_name=` injected. All emitted rows share `transfer_id`, `template_name`, and `parent_transfer_id` — the AB.2.3 matview reads cardinality=1 for healthy firings.

**Plants + scenario (leg 5 of the cross-cutting contract).** Two new dataclasses in `seed.py`:
- `TwoTemplateChainPlant` — healthy two-template chain firing (parent leg row + child template leg firings all agreeing on parent_transfer_id). Positive demo coverage.
- `ChainParentDisagreementPlant` — 2 leg rows sharing one child `transfer_id` + `template_name` but with different `transfer_parent_id` values (synthetic ETL bug). Surfaces the AB.2.3 matview violation.

`auto_scenario` derives `_pick_two_template_chain_inputs` to find any chain whose singleton child resolves to a TransferTemplate; when found, plants both kinds (days_ago 2 vs 1 for visual separation). All 3 densifier + ScenarioPlant pass-through sites carry the new fields. `tests/l2/spec_example.yaml` declares `ReconciliationLeg → MerchantSettlementCycle` (rail→template singleton chain). `tests/l2/sasquatch_pr.yaml` declares the real-world `CustomerFeeAccrual → InternalTransferCycle` chain (3-leg-rail child template demonstrating fan-out: Debit / Credit / SuspenseClose all sharing one Transfer).

**Dashboard wiring.** `<prefix>_todays_exceptions` matview gains a UNION ALL branch projecting `check_type='chain_parent_disagreement'`, `child_template_name AS rail_name`, `distinct_parent_count AS magnitude`. Today's Exceptions surfaces violations alongside the other L1 invariants automatically — no dedicated sheet (the violation is keyed on transfer_id not (account, day), and the analyst's drill path is straight to the Transactions sheet via the conflicting parent ids).

**Studio editor (leg 2 of the contract).** Pre-wired from Z.A lineage — chain card's `parent` + `children` fields already source `rails_or_templates` (verified at `_studio_editor_routes.py:438-465`). Operators declare a template-as-chain-child entirely through the UI; no code change needed for AB.2.

**Topology + diagram.** Pre-wired — `_chain_edges` in `topology.py:546-571` resolves `parent_id` / `child_id` via `_template_id(name) if name in template_names else _rail_id(name)`, so rail and template endpoints both render correctly. No code change.

**Fuzzer (leg 6).** Pre-wired — `_build_chains` already picks children from `state.rail_names + state.transfer_template_names`. Measured: 37% of generated chains carry a template child across 20 seeds (PLAN target was ~25%). Added `"chain_with_template_child"` to the meta-guard's saw-set so the fuzz axis explicitly enforces non-zero coverage.

**Docs.** `concepts/l2/chain.md` gains "Template-as-chain-child (AB.2)" section documenting first-firing-wins, the disagreement L1 violation, and the auto-derived metadata key. New `walkthroughs/customization/how-do-i-chain-two-templates.md` worked example (CustomerFeeAccrual → InternalTransferCycle): operator's story, yaml diff, verify path, 3 "don't" rules. `L1_Invariants.md` gains section 8 (the SHOULD-constraint blockquote + columns + what-to-do + sasquatch fixture surface example) which AA.C.3 parser auto-picks. `INVARIANT_KIND_TO_SHEET`, `_L1_KIND_TO_SHEET_ID`, `_DISPLAY_ORDER`, and `test_bundled_invariants_yields_expected_kinds` all updated to know about the new kind. mkdocs.yml Customization nav updated.

**Tests.** 2 new schema-shape tests (`test_chain_parent_disagreement_view_ab2_shape` + `test_todays_exceptions_includes_chain_parent_disagreement_branch`). 1 new validator-derived test (`test_ab2_two_template_chain_propagates_parent_id_to_every_leg_rail`). Tightened `test_auto_scenario_against_spec_example_covers_all_six_plant_kinds` to assert both AB.2 plant tuples non-empty. Sasquatch chain count assertion updated 3 → 4 with template-as-child positive check. Fuzz meta-guard `chain_with_template_child`. Verified at 2967 passed across unit + data + schema + json + cli + audit + docs layers; live PG + AWS QS verify happens when CI runs the e2e + agreement leg on this tag.

**Re-locked seeds:** `tests/data/_locked_seeds/spec_example.{postgres,oracle,sqlite}.sql` regenerated to include the planted two-template chain firings + ChainParentDisagreementPlant rows.

**CI deadlock flake fix (bundled).** PG integration-pg job was intermittently failing `test_dataset_sql_parses_and_executes` with `DeadlockDetected` on 3 datasets (app-info-matviews / daily-statement-transactions / transactions). Root cause: psycopg's default non-autocommit mode held an implicit transaction open after each `SELECT * FROM (<sql>) WHERE 1=0`, accumulating `AccessShareLock` on every matview the SELECT touched across all parametrized tests in the module. With pytest-xdist running multiple workers, two workers' cumulative lock sets could intersect with an in-flight `REFRESH MATERIALIZED VIEW` (or autovacuum / autoanalyze taking `AccessExclusiveLock` briefly) and PG's deadlock detector killed one transaction. Fix: set `conn.autocommit = True` on the module-scoped `smoke_conn` fixture — each statement is its own implicit transaction; locks drop the instant the SELECT returns. SQLite is effectively autocommit at the statement level already (hence the `hasattr(conn, "autocommit")` guard). Pre-existing flake from before AB.2.
