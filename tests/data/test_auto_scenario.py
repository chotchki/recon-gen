...
def test_auto_scenario_against_spec_example_covers_all_six_plant_kinds(
    ...
    # AB.3.5.spec (2026-05-19) — spec_example carries the XOR-grouped
    # SettlementTimingCycle template, activating both AB.3 plants
    # (missed-firing + overlap). Each emits ONE Transfer surfacing on
    # the AB.3.3 xor_group_violation matview.
    assert len(sc.xor_variant_missed_firing_plants) == 1
    assert len(sc.xor_variant_overlap_plants) == 1
    # AB.4.5.spec (2026-05-19) — spec_example carries the
    # BatchPayoutTrigger → BatchedPayoutBatch fan-in chain
    # (expected_parent_count=2), activating all 3 fan-in plants:
    # healthy (parent_count=2), missing-parent (parent_count=1),
    # extra-parent (parent_count=3). Each emits ONE Transfer
    # surfacing on the AB.4.7 fan_in_disagreement matview.
    assert len(sc.fan_in_chain_plants) == 1
    assert len(sc.fan_in_chain_missing_parent_plants) == 1
    assert len(sc.fan_in_chain_extra_parent_plants) == 1
    # Permitted omissions: TT plants whose first leg_rail isn't TwoLeg
    # (the AB.3.5.spec / AB.3.5b XOR plants are no longer omitted;
    # AB.4.5.spec fan-in plants are no longer omitted either).
