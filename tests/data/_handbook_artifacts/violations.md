...
Generated by `recon-gen docs export --surface=violations` — do not hand-edit. Source of truth is `src/recon_gen/common/l2/plant_registry.py::PLANT_REGISTRY` walked through `recon_gen.common.html._studio_training_v2.resolve_section` (BU.0 Lock 8). One `##` block per kind carries the canonical `short_statement` + `what_to_do` strings the Trainer's `/training/` v3 cards render — deep-linkable from the BTa.1 / BX.12 side-panel system via slug anchors (`#<kind-as-kebab>`). Byte-identity verifier per BU.0 Lock 9.

Row count: **26** kinds.

## Contents

- [Unmatched rail_name](#phantom-rail) — `phantom_rail`
- [Unmatched template_name](#phantom-template) — `phantom_template`
- [Missing required metadata key](#missing-metadata-key) — `missing_metadata_key`
- [Sub-ledger drift](#drift) — `drift`
- [Parent-account roll-up drift](#ledger-drift) — `ledger_drift`
- [Non-negative balance](#overdraft) — `overdraft`
- [Per-direction flow cap](#limit-breach-outbound) — `limit_breach_outbound`
- [Per-direction flow cap](#limit-breach-inbound) — `limit_breach_inbound`
- [Cadence-aware missing balance](#balance-cadence-gap) — `balance_cadence_gap`
- [Per-rail pending aging](#stuck-pending) — `stuck_pending`
- [Per-rail unbundled aging](#stuck-unbundled) — `stuck_unbundled`
- [Two-template chain Parent disagreement](#chain-parent-disagreement) — `chain_parent_disagreement`
- [Multi-mode template variant XOR violation](#xor-group-missed) — `xor_group_missed`
- [Multi-mode template variant XOR violation](#xor-group-overlap) — `xor_group_overlap`
- [Fan-in chain parent-set mismatch](#fan-in-missing-parent) — `fan_in_missing_parent`
- [Fan-in chain parent-set mismatch](#fan-in-extra-parent) — `fan_in_extra_parent`
- [Chain XOR alternation violation](#multi-xor-missed) — `multi_xor_missed`
- [Chain XOR alternation violation](#multi-xor-overlap) — `multi_xor_overlap`
- [Supersession Audit](#supersession-audit) — `supersession_audit`
- [Uncovered rail](#uncovered-rail) — `uncovered_rail`
- [Uncovered template](#uncovered-template) — `uncovered_template`
- [Expected EOD](#expected-eod-balance-breach) — `expected_eod_balance_breach`
- [Chain Orphans](#chain-orphan) — `chain_orphan`
- [Dead Bundles Activity](#dead-bundles-activity) — `dead_bundles_activity`
- [Dead Metadata Declarations](#dead-metadata) — `dead_metadata`
- [Dead Limit Schedules](#dead-limit-schedule) — `dead_limit_schedule`

## Unmatched rail_name {#phantom-rail}

**Kind:** `phantom_rail`

### Short statement

Each row is a posted transaction whose `rail_name` doesn't resolve to any declared `Rail.name` in the L2 yaml. The check groups `<prefix>_transactions` by `rail_name`, then filters to values not in the L2's declared-rails set. The card shows the offending rail name + posting count + a sample `transaction_id` so the operator can grep their source ETL log.
...
**Section reference:** `L2TriageGapSection["unmatched_rail"]`

## Unmatched template_name {#phantom-template}

**Kind:** `phantom_template`

### Short statement

Each row is a posted transaction whose `template_name` doesn't resolve to any declared `TransferTemplate.name`. Same shape as unmatched_rail but on the template axis. The card shows the offending template name + posting count + sample transaction_id + the declared-templates list for context.
...
**Section reference:** `L2TriageGapSection["unmatched_template"]`

## Missing required metadata key {#missing-metadata-key}

**Kind:** `missing_metadata_key`

### Short statement

Each row is a `(template_name, metadata_key)` pair where the template declares the key as required (via its `transfer_key` fields) but at least one posting tagged with that template landed without the key in its metadata JSON. The card shows the count of missing-key postings + the total postings for that template + a sample transaction_id.
...
**Section reference:** `L2TriageGapSection["missing_metadata_key"]`

## Sub-ledger drift {#drift}
...
**Section reference:** `InvariantSection["drift"]`

## Parent-account roll-up drift {#ledger-drift}
...
**Section reference:** `InvariantSection["ledger_drift"]`

## Non-negative balance {#overdraft}
...
**Section reference:** `InvariantSection["overdraft"]`

## Per-direction flow cap {#limit-breach-outbound}
...
**Section reference:** `InvariantSection["limit_breach"]` [Outbound direction]

## Per-direction flow cap {#limit-breach-inbound}
...
**Section reference:** `InvariantSection["limit_breach"]` [Inbound direction]

## Cadence-aware missing balance {#balance-cadence-gap}

**Kind:** `balance_cadence_gap`

### Short statement

For every Account whose `balance_cadence` is `explicit_daily`, every business day in scope SHOULD have an emitted `daily_balances` row. For Accounts on `sparse`, every business day with transaction postings SHOULD have an emitted balance row to reconcile against.
...
**Section reference:** `InvariantSection["balance_cadence_gap"]`

## Per-rail pending aging {#stuck-pending}
...
**Section reference:** `InvariantSection["stuck_pending"]`

## Per-rail unbundled aging {#stuck-unbundled}
...
**Section reference:** `InvariantSection["stuck_unbundled"]`

## Two-template chain Parent disagreement {#chain-parent-disagreement}
...
## Multi-mode template variant XOR violation {#xor-group-missed}
...
**Section reference:** `InvariantSection["xor_group_violation"]` [Missed-firing variant]

## Multi-mode template variant XOR violation {#xor-group-overlap}
...
**Section reference:** `InvariantSection["xor_group_violation"]` [Overlap-firing variant]

## Fan-in chain parent-set mismatch {#fan-in-missing-parent}
...
## Fan-in chain parent-set mismatch {#fan-in-extra-parent}
...
## Chain XOR alternation violation {#multi-xor-missed}
...
**Section reference:** `InvariantSection["multi_xor_violation"]` [Missed-firing variant]

## Chain XOR alternation violation {#multi-xor-overlap}
...
**Section reference:** `InvariantSection["multi_xor_violation"]` [Overlap-firing variant]

## Supersession Audit {#supersession-audit}
...
**Section reference:** `InvariantSection["supersession_audit"]`

## Uncovered rail {#uncovered-rail}

**Kind:** `uncovered_rail`

### Short statement

Each row is an L2-declared `Rail.name` with zero postings in `<prefix>_transactions`. The Coverage Rails panel renders the rail as ✗ (`declared but no rows`). Pulls from the same data slice as the L2FT Dead Rails hygiene check — they're two surfaces on the same underlying check.
...
**Section reference:** `L2TriageGapSection["uncovered_rail"]`

## Uncovered template {#uncovered-template}

**Kind:** `uncovered_template`

### Short statement

Each row is an L2-declared `TransferTemplate.name` with zero postings tagged with that `template_name`. Coverage Templates panel renders the template as ✗. Inverse of `unmatched_template` — that one means rows exist for an undeclared template; this means the template exists but no rows cite it.
...
**Section reference:** `L2TriageGapSection["uncovered_template"]`

## Expected EOD {#expected-eod-balance-breach}
...
**Section reference:** `L2FTExceptionSection["chain_orphans"]`

## Dead Bundles Activity {#dead-bundles-activity}

**Kind:** `dead_bundles_activity`

### Short statement

Each row is an `(aggregating_rail, bundle_target)` pair the L2 declared via `Rail.bundles_activity` that the runtime never matched — no posting carries `rail_name = bundle_target`. Per Z.B, every `bundles_activity` ref is an Identifier rail name (no transfer_type form remains).
...
**Section reference:** `L2FTExceptionSection["dead_bundles_activity"]`

## Dead Metadata Declarations {#dead-metadata}

**Kind:** `dead_metadata`

### Short statement

Each row is a `(rail, metadata_key)` pair the L2 declared via `Rail.metadata_keys` that no posting carries a non-null value for in the window. Each declared pair gets its own SQL fragment in the UNION ALL; the path is the static `$.<key>` JSONPath against `<prefix>_current_transactions.metadata`.
...
**Section reference:** `L2FTExceptionSection["dead_metadata_declarations"]`

## Dead Limit Schedules {#dead-limit-schedule}

**Kind:** `dead_limit_schedule`

### Short statement

Each row is a `(parent_role, rail_name)` LimitSchedule cell with zero outbound debit flow against it in the window. The NOT EXISTS clause checks `current_transactions` for any Debit posting matching the role + rail. A cap nobody routes against can't ever fire as a limit-breach.
...
**Section reference:** `L2FTExceptionSection["dead_limit_schedules"]`
