...
def test_bl_2_default_date_range_applies_on_empty_url_params(
    aiosqlite_pool: AsyncConnectionPool,
) -> None:
    """BL.2 — when the analysis carries a ``default_universal_date_range``
    and the URL's ``date_from`` / ``date_to`` are empty (initial page
    load OR user-cleared submit), the fetcher substitutes the
    analysis's default values before binding the SQL.

    Mirrors QS's "analysis-level TimeRangeFilter parameter defaults
    apply on first render" behavior. Without this, App2 saw empty
    URL params → dataset's match-all sentinel → all-spine read.

    Test exercises the substitution by planting a SQL fragment that
    surfaces the bind value in the output (rather than asserting on
    the actual narrowing — the actual narrowing is a SQL-correctness
    concern handled by the dataset's WHERE clauses)."""
    from recon_gen.common.as_of_frame import AsOfFrame
    from recon_gen.common.tree.date_view import DateView

    # Plant the analysis-level default + a SQL that echoes the
    # ``:date_from`` bind into a comparable column.
    register_sql(
        "x2g-test-ds",
        # SQLite: text(`:date_from`) returns the bound value as a
        # row each invocation. Sum amount narrowed to rows where
        # ``status`` happens to be 'open' — keeps the result stable.
        "SELECT status, amount FROM t WHERE :date_from <> ''",
    )
    app, _ds_node = _build_app_with_visuals()
    assert app.analysis is not None
    frame = AsOfFrame.locked(window_days=7)
    app.analysis.default_universal_date_range = DateView(frame=frame)

    fetcher = make_tree_db_fetcher(
        app, _TEST_CFG_SQLITE, pool=aiosqlite_pool,
    )
    # Empty URL params → fetcher substitutes the analysis default →
    # the ``:date_from <> ''`` clause becomes true → 4 rows returned
    # (sum = 100 + 50 + 200 + 25 = 375).
    out = _run_fetcher(fetcher, VisualId("v-kpi"), {})
    assert out["values"][0]["value"] == 375

    # Without the BL.2 substitution (no analysis default) the same
    # SQL with empty ``date_from`` would short-circuit to 0 rows.
    # Sanity gate: re-build the fetcher without the default and
    # confirm the bind layer falls through to the sentinel.
    app.analysis.default_universal_date_range = None
    fetcher_no_default = make_tree_db_fetcher(
        app, _TEST_CFG_SQLITE, pool=aiosqlite_pool,
    )
    out_no_default = _run_fetcher(
        fetcher_no_default, VisualId("v-kpi"), {},
    )
    # KPI on an empty result set renders 0 (None gets shaped to 0).
    assert out_no_default["values"][0]["value"] in (0, None)


def test_bl_2_user_picked_date_range_overrides_default(
    aiosqlite_pool: AsyncConnectionPool,
) -> None:
    """BL.2 — when the URL params carry non-empty ``date_from`` /
    ``date_to`` (user picked a range via Flatpickr), the analysis's
    default range is IGNORED. User-supplied always wins."""
    from recon_gen.common.as_of_frame import AsOfFrame
    from recon_gen.common.tree.date_view import DateView

    register_sql(
        "x2g-test-ds",
        "SELECT status, amount FROM t WHERE :date_from = '2025-01-01'",
    )
    app, _ds_node = _build_app_with_visuals()
    assert app.analysis is not None
    # Analysis default is some other date — it should NOT be picked.
    frame = AsOfFrame.locked(window_days=7)
    app.analysis.default_universal_date_range = DateView(frame=frame)
    ...
    # User supplies the literal the SQL is gated on → returns rows.
    out = _run_fetcher(
        fetcher, VisualId("v-kpi"),
