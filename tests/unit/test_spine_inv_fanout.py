...
from recon_gen.common.l2.primitives import CREDIT, SCOPE_INTERNAL
...
def _refresh(conn: duckdb.DuckDBPyConnection) -> None:
    ...
    execute_script(
        cur,
        refresh_matviews_sql(instance, prefix=_PREFIX, dialect=Dialect.DUCKDB),
        dialect=Dialect.DUCKDB,
    )
    conn.commit()
...
def test_emit_credits_recipient_n_times() -> None:
    ...
    assert all(r[2] == SCOPE_INTERNAL for r in recipient_rows)
