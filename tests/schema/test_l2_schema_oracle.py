...
class TestOracleConstructsPresent:
    ...
    @pytest.mark.parametrize("pattern", [
        # Type names
        # CB.17.i — `GENERATED BY DEFAULT` (vs ALWAYS) so V-overlay
        # clone INSERT-with-supplied-id works without ORA-32795.
...
def test_refresh_matviews_sql_oracle_uses_dbms_mview() -> None:
    ...
    # 21 matviews refresh in dependency order: 2 current_* + 2 helpers
    # (computed_*) + 1 CL.5 (effective_balances) + 11 L1 invariants
    # (drift / ledger_drift / overdraft / expected_eod_balance_breach /
    # limit_breach / stuck_pending / stuck_unbundled /
    # chain_parent_disagreement [AB.2.3] / xor_group_violation [AB.3.3] /
    # fan_in_disagreement [AB.4.7] / multi_xor_violation [AB.6.5]) + 1
    # derived (transfer_parents [AB.4.3]) + 2 dashboard-shape
    # (daily_statement / l1_exceptions) + 2 Investigation matviews.
    assert sql.count("BEGIN DBMS_MVIEW.REFRESH(") == 21
    assert sql.count("BEGIN DBMS_STATS.GATHER_TABLE_STATS(") == 21
