...
a seeded container (the prior RECON_GEN_E2E env-gate retired in DJ.1).
"""
...
def test_l2ft_rail_dropdown_selection_refetches_with_param(
    ...
    _calls_log.clear()
    # CR.x — this test's contract is the WIRE SHAPE on refresh
    # (param_pL2ftRail serialized as single value, not list-repeated).
    # That's orthogonal to typeahead loading mechanics, so we
    # inject a synthetic option + setValue + dispatch change directly,
    # bypassing the focus → preload → settings.load chain. Tests
    # that DO care about the per-query server fetch path use
    # App2Driver.typeahead_filter(label, query) — exists, tested by
    # the picker-search-and-find suite, NOT this wire-shape test.
    # Per feedback_build_verbs_not_skip: the verb exists; this test's
    # contract just doesn't need it.
    page.evaluate(
        """() => {
