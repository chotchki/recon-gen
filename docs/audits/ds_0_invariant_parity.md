...
| 12 | `inv_pair_rolling_anomalies` | probabilistic | :3980 (base-table read :4002 — supersession-BYPASSING by construction; AT.0 ceiling :1512-1516) | matview only | NONE — excluded from residual typing by name; the exact-ℚ law + ε contract landed as `common/spine/anomaly_contract.py` (DS.4, see `ds_4_anomaly_tolerance_contract.md`) | own tolerance sweep (`test_ds4_anomaly_tolerance.py`), not a gate domain — exact comparator doesn't fit; the integer window layer gets metamorphic laws (DS.3.6) |
...
PROBABILISTIC gets no vectors — excluded from residual typing by name. DS.4 landed its contract: exact-ℚ z² law + ε-tolerance + band-edge adjacency in `common/spine/anomaly_contract.py`, waiver doc at `ds_4_anomaly_tolerance_contract.md` (the min-n floor and stddev=0 guard turned out EXACT on both sides, not tolerance-checked).
