...
## The locks (re-stated for traceability)

Per `PLAN.md::Phase CN`:

1. **`CellAccentText` ↔ at least one `DATA_POINT_CLICK` drill writes from the column.**
2. **`CellAccentMenu` ↔ at least one `DATA_POINT_MENU` drill writes from the column.**
3. **Column with BOTH triggers writing from it → `CellAccentMenu` wins** (tint cue subsumes the accent-text cue — the analyst learns "this cell has a menu drill" and finds the left-click is also wired without needing a separate visual).
4. **A column with NO drill writing from it should NOT carry a CellFormat** (the visual cue implies a click affordance that doesn't exist).
