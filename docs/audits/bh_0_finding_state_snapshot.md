...
|---|---|---|---|
| **#1** | Daily Statement Drift KPI ≠ narrative formula | **RED** (revised 2026-05-25 post-user-share) | TWO independent root causes: **(a)** matview `net_flow` formula bug (`schema.py:2502-2504`) uses `SUM(CASE WHEN Credit THEN amount_money ELSE -amount_money END)` — assumed v5's unsigned amount, but v6's `amount_money` is already signed (Credit positive, Debit negative). The `-amount_money` for Debit rows over-flips → `net_flow = credits + abs(debits)` = gross magnitude, not signed net. Matview's `drift` column then = `closing − (opening + gross)` = wrong by `2 × abs(debits)`. Verified on `cust-0011-snb 2026-02-24`: matview shows drift=-46,986.60, correct formula gives drift=0. **(b)** Cross-day `.max()` over multi-row dataset when date picker is broken (#2 dependent) — `.max()` per column independently picks from different rows → KPIs don't tie. Fix (a): drop the CASE → `SUM(tx.amount_money) AS net_flow` (one-line in `schema.py`). Fix (b): #2 (date picker). **BG.2's narrative-formula assertion is internally consistent right now** because BOTH the matview's `drift` AND matview's `net_flow` are wrong together — the assertion compares `matview.drift` to `matview.closing - (matview.opening + matview.net_flow)` and that holds. The honest narrative-formula gate has to compare against either correctly-computed `net_flow` (drop the CASE) OR a different ground-truth like `SUM(amount_money) FROM current_transactions WHERE account_id=… AND business_day=…`. |
