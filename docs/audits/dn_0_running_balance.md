...
**Resolution (DN-followup 2026-06-16).** DN.1 initially shipped the UNANCHORED (from-zero) variant. An operator review of the live Daily Statement caught it immediately: a $31.4M concentration account's running balance read `$912.70 → -$316,695.79`, obviously wrong next to the $31.4M opening / $31.08M closing. Corrected to the locked ANCHORED variant — `running_balance = COALESCE(opening_cents, 0) + SUM(amount_money) OVER (...)` in BOTH `_daily_statement_transactions_sql` (dashboard) and `_query_daily_statement_walks` (audit/PDF), reusing the same DN.3 opening correlated subquery against `<prefix>_current_daily_balances`. The last leg now lands on `closing_balance_recomputed` (opening + posting-implied net); any gap to `closing_balance_stored` is the planted reconciliation drift the Posting Drift KPI surfaces (verified live: ZBASweep `+912.70` → `$31,401,689.71`, ConcToFRB `-317,608.49` → `$31,084,081.22`).
...
**Drift KPI false-alarm risk.** The summary KPI `closing_balance_recomputed` is `opening_balance + total_credits - total_debits` (per `<prefix>_daily_statement_summary` matview). If the row-level running balance lands at value X on the last leg, then (ANCHORED variant, DN-followup 2026-06-16) `running_balance(last_leg) ≡ closing_balance_recomputed` must hold. DN.5 asserts this — if the window function and the matview disagree on the closing-of-day arithmetic, that's a real arithmetic bug worth surfacing (or, more likely, an order-of-rows bug between the matview's SUM-per-day and the window-function's running sum).
