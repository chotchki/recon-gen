...
## Starting balance (optional sub-feature)

The running balance as defined above starts at 0 + the first leg's signed amount. This is correct **relative arithmetic** — the running balance shows how the day's activity stacks — but it is not the **statement-shaped** view a CPA expects, which anchors the first row to the account's prior end-of-day balance.

Source: `<prefix>_current_daily_balances` for `(account_id, balance_date = pL1DsBalanceDate - 1 day)` gives yesterday's stored EOD balance. A single roundtrip via the existing pool (single-row LIMIT 1 query) is cheap.

Two UX shapes — **operator confirm needed at DN.0 lock**:

| Shape | Pro | Con |
|---|---|---|
| **(a) Subtitle badge** on the Posted Money Records visual: "Opening balance: $X (as of YYYY-MM-DD)". | Cleanest visually — table rows stay activity-only. Drop-in: just extend the visual's subtitle. | Subtitle is static text on QS — substituting a per-account-per-day value requires the dataset parameter to be readable into the subtitle template, which `[[project_qs_text_box_rich_formatting]]` indicates works via `<<$param>>` expression injection but only against the param value, not against a dataset cell. We'd need either a second single-row dataset that joins the picker params to the prior-EOD balance lookup, OR a parameter that's set by drill (operator-driven, not auto-computed). Verify at DN.0-lock or split off as DN.3.a. |
| **(b) Synthetic opening row** prepended to the table with rowtype "Opening Balance" and `running_balance` populated from the prior EOD. | Renders directly in the table; QS + App2 see it identically (just another row in the projection). No subtitle plumbing. | Breaks the implicit invariant that every row in the projection is a real `transaction_id`. Schema cost: `transaction_id` becomes nullable on the opening row OR we synthesize a sentinel like `__opening__` and add a column-shape annotation that the visual hides drill affordances on it. Test cost: every test that asserts `len(rows) == seed_leg_count` must account for the synthetic row. |

**Default recommendation (DN.0 author): shape (a), subtitle badge.** Lower row-shape coupling, lower test churn, lower contract drift. The "second single-row dataset" cost is real but bounded — `<prefix>_daily_statement_opening_balance` dataset, projection `(account_id, business_day, opening_balance)`, narrowed to the picked (account, day) via the same `pL1DsAccount` / `pL1DsBalanceDate` parameters; subtitle template reads from it via QS's expression-injection vocabulary on the visual subtitle. App2 side: subtitle template reads from the dataset's first row.

**Open question (DN.0):** does QS's subtitle template syntax support reading from a dataset cell (`<<$DatasetName.column>>`) or only from analysis parameters (`<<$paramName>>`)? `[[project_qs_text_box_rich_formatting]]` documents the rich-formatting XML on `SheetTextBox`; the param-injection capability on table-visual subtitles specifically may be narrower. If only analysis params work in subtitles, shape (a) requires the subtitle to read from a dataset-parameter calc rather than a dataset cell — workable but more wiring. If neither works, fall back to shape (b) or push the opening balance into a sibling visual (a 1-row "Opening Balance" KPI tile above the table).

**Operator confirm needed before DN.3 starts.** Suggested resolution at DN.0 lock: pick shape (a) as the default; DN.3 implements (a) and if the QS subtitle vocabulary doesn't allow it, the leaf splits into DN.3.a (subtitle path) or DN.3.b (KPI tile fallback).
...
- [ ] **DN.3 — Starting balance: subtitle badge (or synthetic-row fallback).** Operator-confirm-required before this leaf starts. Default shape (a) — opening-balance dataset + subtitle template injection. Falls back to (b) or (c) per the open question above. May split into DN.3.a / DN.3.b depending on QS subtitle template capability.
...
- [ ] **DN.5 — Tests.** (i) Unit cross-dialect SQL-emit test for `_daily_statement_transactions_sql` covering PG / Oracle / DuckDB — asserts the produced column sequence matches Python-computed `accumulate(signed_amounts)`. (ii) Unit test asserting `running_balance(last_leg) ≡ closing_balance_recomputed - opening_balance` against a planted fixture. (iii) E2E both renderers (`[qs, app2]`) — assert column present + arithmetic correct on a known seeded account-day. (iv) Audit PDF parity test extends to include running_balance.
