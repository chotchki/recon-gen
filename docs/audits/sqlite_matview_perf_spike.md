...
### Candidate C — DuckDB swap

**Idea.** Run the matview refresh against DuckDB instead of SQLite. DuckDB is in-process (same offline-iteration story), has a vectorized executor + cost-based optimizer (handles correlated subqueries gracefully), and reads/writes SQLite files natively via the `sqlite` extension or through its own format.

**Effort.** Large. ~2-3 weeks. The dialect surface is non-trivial: DuckDB is *mostly* PG-compatible but diverges on JSON path syntax (DuckDB uses `->` and `->>`, not SQL/JSON `JSON_VALUE` — the codebase's "portable SQL/JSON path" decision was specifically about SQLite + Oracle + PG; DuckDB would be a fourth dialect to support). New `Dialect.DUCKDB` enum value; new templates / helpers; new dialect arms in 20+ schema functions; new container fixture in `_dev/runner.py`; new `tests/data/_locked_seeds/<instance>.duckdb.sql`.
...
**Risks.**
- **Dialect proliferation.** Today: 3 dialects, mostly stable. DuckDB adds maintenance surface to every matview template + helper. Every future schema change is now 4-dialect work.
- **DuckDB on-disk format vs. SQLite.** DuckDB's native `.duckdb` files aren't SQLite — Studio's "open the wheel, point at a SQLite file" loop changes. The `sqlite` extension can read/write SQLite, but write performance through that extension is reportedly slower than native. Need a spike on whether Studio's "edit and persist" model works against DuckDB's SQLite-extension path.
- **Sneak risk: SQL/JSON compat.** The codebase already burned a phase (BC.12) on Oracle 19c+ rejecting matviews over JSON_TABLE-of-CLOB. DuckDB has its own JSON dialect quirks. Adding it without a hands-on JSON-path-portability test is a trap.
- The user's `[feedback_rust_influenced_tool_preferences]` makes DuckDB attractive on principle (Rust core, single binary, single Python wheel). That doesn't fix the dialect-surface cost.
...
- Candidate C (DuckDB swap) is appealing on principle (vectorized exec, in-process, Rust core matches the `[feedback_rust_influenced_tool_preferences]` posture) and has the highest absolute ceiling. But the dialect-surface cost is real, and we've already burned a phase (BC.12) on subtle JSON-path-portability issues across dialects. The right time to consider DuckDB is **after** the SQL is clean — if cleanly-written SQL on SQLite still isn't fast enough, then DuckDB's vectorized executor is genuinely the missing piece, not a workaround for unoptimized SQL.
