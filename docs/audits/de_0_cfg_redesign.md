...
> **POST-DW UPDATE (2026-06-28 — dead-config sweep).** QuickSight + the whole AWS deploy path were removed in Phase DW, and the dead-config sweep then deleted the AWS cfg surface this spec inventoried. **`cfg.aws` is now a single field — `deployment_name` — plus the `prefixed()` method.** GONE from `cfg.aws`: `account_id`, `region`, `principal_arns`, `extra_tags`, `tagging_enabled`, `qs_disable_pg_ssl`, `pg_cluster_id`, `oracle_instance_id`, `datasource` (the whole `DatasourceConfig` + the `create`/`adopt`/`skip` mode enum locked at DE.0 §"Datasource lifecycle"), the `partition` property, and `dataset_arn()`/`theme_arn()`. GONE from `cfg.auth`: the entire `auth.aws` sub-block (`profile` + `quicksight_user_arn`) — `auth.oidc` + `auth.session` (App2's own login) stay. The matching env vars (`RECON_GEN_AWS_ACCOUNT_ID`/`AWS_REGION`/`DATASOURCE_ARN`/`PRINCIPAL_ARNS`/`AWS_PG_CLUSTER_ID`/`AWS_ORACLE_INSTANCE_ID`) are deleted; only `RECON_GEN_DEPLOYMENT_NAME` survives. **The inventory + datasource-lifecycle + migration sections below are HISTORICAL** — they describe the v13→v14 shape as it stood pre-DW. DE.1+ should treat `cfg.aws = {deployment_name}` as the real current state and NOT redesign the removed AWS fields. A cfg that still carries the old `aws.account_id`/`region`/`datasource` keys loads fine (they're ignored). The `auth.aws_profile`/`quicksight_user_arn` legacy-key migration hints still fire but now point at removed fields — fold their cleanup into DE.
