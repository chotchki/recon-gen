...
1. **CF.0 is probably NOT a DuckDB regression.** `git diff ac855db8 HEAD -- src/recon_gen/common/spine/ src/recon_gen/common/l2/` is empty. The drift / limit_breach_outbound plant code is byte-identical to the v13.1.1 wheel the reviewer tested. CI is green for both kinds on DuckDB against sasquatch_pr at default density. The cold-read framing ("observed on duckdb, suggesting a duckdb-path plant bug") is unverified inference. The selectivity (drift + limit_breach_outbound fail; chain plants work) aligns cleanly with which plants depend on `_pick_external_counter_for_rail` / `_pick_breach_inputs` — **strong evidence the root cause is the reviewer's L2 shape or seed density, not DuckDB**. Need the reviewer's L2 yaml OR a fuzz-seed sweep to confirm.
