...
- The renderer question is **D3 alone vs D3 + ELK as a layout engine** (NOT a Mermaid revisit — the Phase S Mermaid spike was a dead end per the user). D3 keeps its rendering job (it works); the spike's job is to figure out whether d3-force gets us to "legible on `sasquatch_pr`" with tunable knobs, or whether we need ELK to compute positions and feed them into D3 (so D3 still renders, ELK just does the layout math). Don't lock the layout choice until the spike answers.
