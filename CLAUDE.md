...
When a specific test fails (chain run, CI flake, picker bug, dataset SQL exception) and you need to drive the live system to investigate, `./run_tests.sh triage <pytest_nodeid>` is the canonical entry. Replaces every ad-hoc "spin a PG container by hand, edit the QS-side cfg, run `recon-gen json apply --execute`, mint an embed URL, pdb-import" sequence we did pre-DI.

- **What it does.** Layer-infers from nodeid (e.g. `tests/e2e/test_l2ft_*.py` → `qs_browser`; `tests/e2e/app2/` → `app2`; `tests/e2e/db/` → `db`; `tests/unit/` → `unit`). Spins the PG / Oracle / DuckDB container per cfg dialect; the session-scoped `qs_deployed` autouse fixture (in `tests/e2e/conftest.py`) handles QS deploy idempotently when the layer crosses into `AWS_TOUCHING_LAYERS`. Spawns `pytest --pdb <nodeid>` inside a detached GNU `screen` session named `recon-gen-triage`. POLICY 1 enforced by construction: chain and triage both go through `pg_container_url` + `qs_deployed`, so fixture setup is the same code path.
...
- **Pdb gives access to live fixtures.** Inside pdb you have the test's locals plus the driver fixture state — for `qs_browser` tests `driver._page` (live Playwright Page; `.screenshot()`, `.evaluate(js)`, `.locator(...).click()`), `driver._ws_tracker` (QS START_VIS/STOP_VIS WebSocket frame history), `driver._cfg` (the cfg the driver was built with). For `app2` tests the same affordance via `driver.page` + `driver.base_url`. For `db` tier tests: `isolated_cfg` + live cursors. This is the killer feature — you can actually fire `driver.pick_filter('Rail', ['ConcentrationToFRBSweep'])` from pdb and watch the dataset re-query happen.
- **Eager AA.H.6 capture on QS SQL exceptions.** When the `QsEmbedDriver` SQL-exception scanner fires (`tests/e2e/_drivers/qs.py::_assert_no_sql_exception`), the AA.H.6 capture suite triggers eagerly — screenshot.png / dom.html / qs_errors.txt / console.txt / network.txt / ws_frames.txt / db_counts.txt / trace.zip land in `runs/<run-id>/browser/<sanitized_test_id>/` BEFORE pdb opens, so the artifacts reflect the moment of detection (not the post-assertion state). Pdb drops in at the assertion line; locals (`errored`, `context`, `self._page`) are all reachable.
- **Teardown.** `./run_tests.sh triage-down --yes` — kills the screen session, runs `recon-gen json clean --all --execute` against the QS-side cfg the deploy used (idempotent; no-op if nothing matches the `ManagedBy=recon-gen + Deployment=<deployment_name>` tag pair), and stops ONLY the triage-spawned container (anonymous testcontainers name captured at spawn time; `recon-gen-snap-test-*` containers + other unrelated runners stay untouched). `--keep-container` skips the container stop; `--keep-qs` skips the QS sweep.

### Test sequencing + git hooks (`Y.2.gate.d`+`k.5`+`k.7`)

- **Always invoke `./run_tests.sh up_to=<layer>`** — the runner enforces `unit → db → app2 → api → browser` ordering (invoking layer N runs 1..N-1; `unit` runs once as prelude, not per-cell). Direct `pytest` is fine only for iterating on a single test you're actively writing; bare pytest for layered work has shipped silent dashboard failures (Y.2.b SELECT-alias-in-WHERE bug). **For triage of a specific failing test, reach for `./run_tests.sh triage <nodeid>` (see Triage workflow above) instead of bare pytest — the runner sets up the same fixture surface the chain does, drops into pdb on the failure, and the session-scoped `qs_deployed` fixture handles QS deploy automatically.** The `deploy` layer is retired post-DI phase; deploy fires inside pytest via the `qs_deployed` autouse fixture when the test pulls AWS-tagged dependencies.
