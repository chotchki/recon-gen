...
- **Local ≡ CI for QS support.** Same `./run_tests.sh` invocation, same `cfg.auth.aws.profile` → `sts:GetCallerIdentity` → `quicksight:ListUsers` derivation, same QS user (`recon-gen-admin`). The WSL2 self-hosted CI runner has its own long-lived `recon-gen-local`-class IAM keys identical to local — no static `QS_E2E_USER_ARN` secret, no browser-login asymmetry, no test that runs only on one side. If a QS-leg test passes locally, it passes on CI (modulo the actual QS subscription state, which both environments share).
