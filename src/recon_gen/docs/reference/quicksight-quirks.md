...
> The detection machinery these entries drove us to build — active canary + diagnostic ladder + eager failure-capture — is renderer-AGNOSTIC and outlived QuickSight; it's written up as a reusable pattern in [Test-harness quirks → Detecting a renderer that fails silently](test-harness-quirks.md#detecting-a-renderer-that-fails-silently). The entries below are the QS-specific symptoms.
