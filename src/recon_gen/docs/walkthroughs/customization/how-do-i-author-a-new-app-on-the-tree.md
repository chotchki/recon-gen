...
greenfield example — start to finish: five content sheets plus the
App Info canary, a mix of KPI, bar-chart and table visuals,
date-range narrowing pushed into the dataset SQL (no analysis-level
FilterGroup), no cross-app drills. It's the smallest end-to-end app
in the codebase, carrying none of the old boilerplate — per-app
...
- **`src/recon_gen/apps/executives/app.py`** — the worked example.
  Five sheet specs (plus the App Info canary), five populator
  functions, the date-range pushdown wiring and one
  `build_executives_app()` entry point. Read the whole file before
  starting.
- **`src/recon_gen/apps/executives/datasets.py`** — the dataset
  side. Several custom-SQL datasets, one `DatasetContract` per
  shape and one `build_all_datasets()` helper that registers the
  contracts via `register_contract()` as a module-import side effect.
...
- **No `constants.py`.** Sheet IDs are inline in `app.py` (URL-facing,
  ...
  the resolver — you never write them.
...
4. **The tree IS the source of truth.** Tests walk the tree to derive
   expected sets — `tests/json/test_executives.py` is a good template. Don't
...
The tree primitives expose more than this walkthrough surfaces:
...
2. Skim `tests/json/test_executives.py` for the test pattern (walk the
