...
A chain row declares a firing expectation: when ``parent`` fires, one
of its ``children`` SHOULD also fire. That's the whole job — one piece
of L2 hygiene the tool checks against runtime data, asking did the
expected child actually fire when the parent did.
...
transactions but you still want hygiene to check the second one
happened.
...
via the mapping form. The flag lives per-child (the chain-level
``fan_in`` / ``expected_parent_count`` were retired) so one chain can
mix 1:1 and N:1 children naturally — some 1:1 children plus some N:1
children under a single chain. Each child entry may carry two
optional fields:
...
- ``fan_in: true`` — declares the N:1 inversion for THIS child.
  ...
  when ``fan_in=true`` (rail-as-child fan-in is undefined, so the
  loader closes that door at load time).
...
``<prefix>_chain_parent_disagreement`` matview automatically excludes
fan_in template children — they're legitimately multi-parent by
design, so the cardinality-1 check would false-positive every fan-in
firing.

> See [How do I model batched payouts?](../../walkthroughs/customization/how-do-i-model-batched-payouts.md)
> for a worked example.

## Multi-XOR runtime enforcement
...
ONE of the declared children. That's the declaration; the runtime
side is the L1 ``<prefix>_multi_xor_violation`` matview, which flags
...
The matview's CTE skips per-child ``fan_in=True`` entries — their
cardinality is ``<prefix>_fan_in_disagreement``'s job instead. A
mixed-cardinality chain (one fan_in child plus two or more 1:1 XOR
siblings) contributes only the non-fan_in siblings to the multi-XOR
matview.

## Mixed-cardinality chains

Moving ``fan_in`` from chain-level to per-child means one chain can
carry both:
...
That's the worked demo above: every settled cycle picks ONE of three
payout vehicles AND contributes to the week's batched payout. The two
enforcement matviews split the work — `_multi_xor_violation` flags
XOR violations on the three alternatives, `_fan_in_disagreement`
flags batches whose `parent_count ≠ 5` on
