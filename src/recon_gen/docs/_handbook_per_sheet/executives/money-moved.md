...
- `rail_name` — the transfer's declared family (ACH, wire, check, merchant card, on-us internal transfer, etc.). Note: to keep the legend readable, any `rail_name` outside the top 20 by gross-dollar volume is collapsed into a string literal `"Other"`
...
- **Confirm the matview is fresh.** Cross to App Info and check the *Matview Status* table's `<prefix>_transactions` row. If `latest_date` lags the postings you know landed since, the data is stale. The institution refreshes matviews on every ETL load; ad-hoc dashboard hits don't trigger one.
