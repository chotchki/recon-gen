...
- *Transfer Legs (all statuses)* — a separate, raw count of every row in `<prefix>_transactions` (all status values, all legs) as of the end-of-day refresh. This value matches the *App Info* sheet's `<prefix>_transactions` row count exactly. The KPI's subtitle explains why this number differs from Total Transactions: the transaction summary filters to Posted only and groups by transfer, whereas App Info counts raw legs.
