...
def emit_seed(
    ...
    *,
    ...
    ctx = ScenarioContext(
        scenario_id=f"emit-seed-{prefix}",
        prefix=prefix,
        dialect=dialect,
    )
    captured = ctx.compose(cap, *generators, dry_run=True)  # type: ignore[arg-type]: ViolationGenerator → ClaimedAccountsGenerator Protocol narrowing not inferred
    rendered = render_captured_sql(captured, dialect=dialect) if captured else ""
    return (
        f"-- =====================================================================\n"
        f"-- L2 instance: {prefix} — demo seed\n"
        f"-- Generated by recon_gen.common.spine plant adapter (AY.6)\n"
        f"-- Reference date: {scenarios.today.isoformat()}\n"
        f"-- {len(generators)} plant generators composed via "
        f"ScenarioContext.compose(dry_run=True)\n"
        f"-- =====================================================================\n"
        f"{rendered}"
    )
...
def emit_full_seed(
    ...
    *,
    ...
    # AY.4.d — preserve the OLD emit_seed's prose header (some tests
    # assert on the "demo seed" marker for sanity-checking the bundle).
    # The body itself is the spine-rendered SQL; the header documents
    # what produced it + the scenario's plant counts for operator
    # eyeball-diffing.
    plants_header = (
        f"-- =====================================================================\n"
        f"-- L2 instance: {prefix} — demo seed\n"
        f"-- Generated by recon_gen.common.spine plant adapter (AY.4)\n"
        f"-- Reference date: {scenarios.today.isoformat()}\n"
        f"-- {len(generators)} plant generators composed via "
        f"ScenarioContext.compose(dry_run=True)\n"
        f"-- =====================================================================\n"
    )
    plants_sql = f"{plants_header}{rendered}" if rendered else plants_header
