...
def wipe_demo_data_sql(
    ...
    *,
    ...
    real customer data) survive. The dialect-portable scalar extract is
    ``json_value(metadata, '$.source', dialect)`` —
    ``json_extract_string`` on DuckDB and ``JSON_VALUE`` on PG / Oracle.
    The default (``synthetic_only=False``) preserves the ETL-mode
    full-TRUNCATE semantics that the CLI ``data apply --execute`` path,
    the studio Deploy-changes button, and the Trainer reset with
    ``etl_hook`` configured all rely on.
    """
    p = prefix
    header = (
        f"-- =====================================================================\n"
    ...
    if synthetic_only:
        # CZ.3 — standalone-mode safety: delete only rows the seed pipeline
        # stamped via CZ.2 (metadata.source='training'). Unmarked rows are
        # presumed real customer data and survive.
        src_tx = json_value("metadata", "'$.source'", dialect)
        ...
        return (
            ...
            + f"WHERE {src_bal} = 'training';\n"
            + f"DELETE FROM {p}_transactions "
            + f"WHERE {src_tx} = 'training';\n"
        )
    return (
        header
...
def _render_fan_in_disagreement_body(
    ...
    return (
        ...
        f"child_firings AS (\n"
        f"  -- DS.3.3c — the count spine seeds from the CHILD TEMPLATE'S\n"
        f"  -- OWN firings, never from parent claims: a child whose\n"
        f"  -- claims are ALL missing (the worst corruption shape) must\n"
        f"  -- still materialize with parent_count 0. Found by the\n"
        f"  -- DS.3.5 enumeration gate — the old spine built FROM\n"
        f"  -- transfer_parents, so one-missing alarmed while\n"
        f"  -- all-missing was invisible.\n"
        f"  SELECT\n"
        f"    tx.transfer_id AS child_transfer_id,\n"
        f"    MIN(tx.template_name) AS template_name,\n"
        f"    MIN({date_trunc_day('tx.posting', dialect)}) AS business_day\n"
        f"  FROM {p}_current_transactions tx\n"
        f"  WHERE tx.template_name IN\n"
        f"        (SELECT child_template_name FROM fan_in_chains)\n"
        f"    AND tx.status IN ('Posted', 'Pending')\n"
        f"  GROUP BY tx.transfer_id\n"
        f"),\n"
        f"child_parent_counts AS (\n"
        f"  -- Per fired child Transfer: how many DISTINCT parents\n"
        f"  -- contributed (LEFT JOIN against AB.4.3's _transfer_parents\n"
        f"  -- — zero claims counts as ZERO, not absence).\n"
        f"  SELECT\n"
        f"    cf.child_transfer_id,\n"
        f"    cf.template_name,\n"
        f"    COUNT(DISTINCT tp.parent_transfer_id) AS parent_count,\n"
        f"    cf.business_day\n"
        f"  FROM child_firings cf\n"
        f"  LEFT JOIN {p}_transfer_parents tp\n"
        f"    ON tp.child_transfer_id = cf.child_transfer_id\n"
        f"  GROUP BY cf.child_transfer_id, cf.template_name, cf.business_day\n"
        f")\n"
        f"SELECT\n"
...
def _render_multi_xor_violation_body(
    ...
    return (
        ...
        f"),\n"
        f"parent_days AS (\n"
        f"  -- DS.3.3a — the parent's business_day aggregates SEPARATELY\n"
        f"  -- from the child-name dedup: carrying the day inside\n"
        f"  -- fired_children_distinct multiplied the count by |distinct\n"
        f"  -- posting days| (a parent whose legs straddle midnight read\n"
        f"  -- as an overlap of its own single child).\n"
        f"  SELECT parent_transfer_id, chain_parent_name,\n"
        f"         MIN(business_day) AS business_day\n"
        f"  FROM parent_firings\n"
        f"  GROUP BY parent_transfer_id, chain_parent_name\n"
        f"),\n"
        ...
        f"  -- name per (parent, child) — and carries NO day column,\n"
        f"  -- names count once however many days the parent posted).\n"
        f"  SELECT DISTINCT\n"
        ...
        f"JOIN parent_days pd\n"
        f"  ON pd.parent_transfer_id = fcd.parent_transfer_id\n"
        f" AND pd.chain_parent_name = fcd.chain_parent_name\n"
        f"GROUP BY fcd.parent_transfer_id, fcd.chain_parent_name, pd.business_day\n"
